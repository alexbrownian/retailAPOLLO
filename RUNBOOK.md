...
| Use the ungated GET IN / signed readiness (2026-08-17 adoption, notebook 08 §10) | NOTHING TO RUN if the store is fresh — run `python -m analytics.run_analytics --what phases` ONCE after pulling this change (a store written before it lacks the `get_in_nogate[_strict]` columns; the dashboard warns until then). **GET IN is UNGATED by default**: the 120d phase gate was measured (nb08 §10.4) blocking ~3/4 of correct GET IN calls (captured episodes 28 → 116 ungated, at under double the false alarms) because chained episodes re-onset while a name still counts as boomed. The sidebar checkbox **"stricter threshold (price-gate GET IN)"** restores the gated variant; GET OUT keeps its gate ALWAYS (ungated its false alarms double — same section). Each name's price chart now carries the **signed readiness band** below it (one number per day in [-1, +1]: green + = the IN side is live and this close to its cut, red − = the OUT side likewise; the sign follows the phase routing so it can NEVER point both ways — the fix for the both-at-100% radar contradiction, nb08 §10.1/§10.5). The **ETF radar** tab ranks every tracked name by the same signed number, most bearish first. Evidence: `docs/research/nb08_single_dial.json`. **Also shipped 2026-08-17:** the **retail-flow dial** (notebook 08 §9) — `retail_flow` / `retail_flow_disp` columns written by every phases run (~2–4 min extra; walk-forward refit, deterministic) and drawn as a second band under each price chart: green = the crowd is building (measured to LEAD price by 1–3 weeks), red = draining. Trend context only — it cannot fire, gate or re-score a call (fenced by `TestRetailFlowDial`). And **readiness alerts**: every phases run writes `data/processed/readiness_alerts.json` with the THEMES at |signed readiness| ≥ 90% of the cut (themes only — single names never alert, desk 2026-08-17; fresh-scored rows only, 60-day rule), prints them in the run log, and the dashboard banners them above the tabs — the alert file and the bands are cut from the same store so they can never disagree. Fences for the whole package: `TestSignedReadinessAndUngatedGetIn`. |
...
| Read the inflection markers | NOTHING TO RUN — `python update_data.py` writes `inflection` / `inflection_score` into `euphoria_desk.parquet` every run and the price panel draws them as small diamonds on the axis. **An INFLECTION is CONTEXT, not a call**: it means a reversal is more likely than usual in the next two weeks, it fires at tops AND bottoms with no direction, and it hits 9.6% of the time against a 5.6% base. Use it to corroborate — a GET OUT with an INFLECTION beside it is a more interesting GET OUT — never on its own. In the watchlist it is a **column** — how close each name is to an inflection — not a competing sort: the ranking stays on the two real calls, because context should not decide what you look at first. It still cannot fire, gate or re-score a call (fenced by `TestInflectionMarker`). **The signal setting was renamed 2026-08-12 and RETIRED 2026-08-17**: the interface now serves **Standard only** (the F0.5 `*_strict` columns); Relaxed is gone from the sidebar. The stored F1 columns (bare `get_in`/`get_out`) remain in the parquet so every recorded number still reproduces. Threshold frozen in `euphoria_desk_report.json` under `inflection`; re-derive with `--research`. Evidence: `docs/research/inflection_trigger_sweep.json`, prototype in notebook 08. **Known weakness:** it beat its base rate in 4 of the 6 years with data and failed in 2022 and 2023 |
| Switch signal strictness | NOTHING TO RUN — sidebar "signal setting" on the dashboard: **Standard** (F0.5 cut, the DEFAULT, fewer and higher-conviction) or **Relaxed** (F1 cut, roughly twice the calls). **Renamed 2026-08-12** — what was *Strict* is now Standard and what was *Standard* is now Relaxed; the cuts themselves did not move, and the stored columns keep their original names (`*_strict` = F0.5 = Standard, bare = F1 = Relaxed). Both settings fire SHAPED calls (2026-08-09): GET IN only before a name's boom completes, GET OUT only after (the ground truth's own 120d bar), deep re-arm, one call per name per quarter per side, never a GET IN within 21d of a GET OUT. Evidence: `docs/research/alert_shape_sweep.json` |
| Try the EXPERIMENTAL posts-only trigger | NOTHING TO RUN if the store is fresh — sidebar **"trigger"** on the dashboard: **Shipped (crowd + price)** (the DEFAULT) or **Experimental (posts only)**. The experimental pair reads THIRTEEN crowd measurements and no price anywhere — no price features AND no phase gate, so either side may fire any day and the readiness dial shows whichever side is nearer. Every score, marker, cut, watchlist row switches with it (columns `*_xp`; cuts frozen in `euphoria_desk_report.json` under `experimental_price_blind`). **It is the weaker detector and says so**: walk-forward AUROC ~0.57 vs the shipped ~0.73 — it answers "what can the crowd alone see", it is not the desk signal. A store written before 2026-08-14 lacks the columns; run `python -m analytics.run_analytics --what phases` once (the dashboard warns until then). Evidence: `docs/research/nb08_price_blind.json`, research in notebook 08 §8 |
...
| Backfill a Reddit SUBMISSIONS gap (the coverage table shows '.' or a volume collapse) | **`python tools/backfill_reddit.py`** — the resumable runner (defaults to the 2023-04-01 → 2026-01-01 drought, one calendar month per chunk). `--estimate` pulls one probe day and prints a runtime measured on THIS machine; `--status` lists what is done and what is left; `--max-chunks N` stops early. Each chunk is its own fetcher process, so a finished chunk is durable and an interrupted run resumes at the first unfinished one — **the bare fetcher is not resumable**, it writes to a `.tmp` and only renames at the very end, so killing it discards the whole run. A chunk where any subreddit gave up is left on the to-do list rather than silently marked done. Underneath it is still `python ingestion/fetch_reddit_arctic.py --backfill START END` — the ordinary incremental window is `max(lookback, watermark)` and therefore **cannot walk backwards**, so a historical gap needs an explicit window. The watermark is ignored AND left unchanged, so the next normal run still resumes from the present; dedup is by post id, so overlapping an already-covered span is harmless. Then fold it in: `python update_data.py --skip-fetch` (rebuilds aggregates + analytics from the new raw files), and re-open research: `python -m analytics.run_analytics --what phases --research`. Re-check `--- DATA COVERAGE ---` in the next run's log. Progress ledger: `data/reference/reddit_backfill_progress.json`. **SPEED WORK 2026-08-21** (the 2024-08 chunk had taken 712 min and still lost 3 subreddits): the fetcher now uses `limit=auto` (Arctic returns 100-1000 rows a page instead of 100), `fields=` restricted to the EIGHT fields anything downstream reads, rate-limit-header pacing instead of a flat 1s sleep per page, a reused HTTP session, and 4xx handling that cannot burn 200s per occurrence. Measured against a stand-in server on identical data: **6.5x fewer requests, 9.2x less downloaded, byte-identical post set**; and a server answering 422 costs **0.3 min of backoff instead of 10.1 min** across 3 subreddits, with the response body now logged so the real cause is visible. `--workers N` fetches N subreddits at once (default 4 in the wrapper); `--subreddits a,b,c` restricts scope - the biggest lever if you want SOME history now: wallstreetbets alone retains 24% of covered name-days, +valueinvesting+stocks 59%, +dividends+bogleheads 76%. `--no-fast` restores the old behaviour for comparison. The DAILY live run is unchanged: fast mode is on for `--backfill` only. **The chunk ledger is keyed by window AND subreddit set** (fixed 2026-08-21): a chunk is only 'done' for the subreddits it actually fetched, so coming back later to add more subreddits correctly re-runs the window instead of silently skipping it and reporting success. Re-running the SAME set still skips; the set tag is order-independent. Entries written before this change carry no set tag and are treated as not-done for any subset, which may re-fetch some posts - harmless (dedup is by post id) but it costs time. |
...
| Live fold is slow after a backfill | **FIXED 2026-08-21 - nothing to run.** `ingestion/append_live_abstracted.py` used to glob EVERY file in `data/raw/RedditLive/` and read all of them on every run. After the 2026-08-21 backfill that folder held 83 files / ~600 MB / ~1.9 M posts, so an ordinary `python update_data.py` sat at 'folding live raw' for many minutes re-reading history it then DROPPED at the LIVE_START filter. It now keeps a per-file ledger (`files_scanned` in `data/reference/abstracted_live_meta.json`): a file whose NEWEST post predates LIVE_START can never contribute a row, so it is skipped without being opened. The skip is invalidated if the file changes on disk (size or mtime) or if LIVE_START moves - both fenced by tests. Measured on a 44-file fixture: read stage 1.25s -> 0.03s (**48x**), with the in-window rows byte-identical. It deliberately does NOT skip on 'seen these ids': `seen_ids` is capped at 300k, so an id can be evicted and that test would drop rows it should fold. |
| Rebuild the ONSET detector + DESK signals (GET IN / GET OUT) | `python -m analytics.run_analytics --what phases` — LIVE mode: episode catalog + today's scores/alerts at the frozen thresholds (seconds), including `euphoria_desk.parquet` (scored by the SHIPPED desk model - since 2026-08-07 the tournament-selected logit+GBM ensemble at its frozen probability cuts; the rules path remains the automatic fallback if a frozen record ever names it); add `--research` for walk-forward scorecards + threshold re-freeze. **Since 2026-08-21 the model FAMILY IS PINNED** to `ens` (the logit + monotone-GBM rank ensemble) via `DESK_MODEL_FAMILY` in `src/config.py`, so `--research` fits **2 models, not 16**: ~2 min of fitting instead of ~24. It had selected `ens` on every pass since 2026-08-07, and re-deciding it on every fold was spending most of the run re-deriving a settled conclusion. Pinning does NOT weaken validation - the family is still fitted walk-forward, still judged against the episode ledger, and still reports capture / FA / AP. **Re-open the tournament** whenever the feature bank, the label definitions or the universe change, because the pinned family was chosen under the old ones: set `DESK_MODEL_FAMILY = None`, or for one run `$env:DESK_MODEL_FAMILY=""` (PowerShell) / `DESK_MODEL_FAMILY= python -m ...` (bash). Fences: `TestPinnedModelFamily` |
| Re-run the research notebooks | for each notebook N: `python -m jupytext --to ipynb notebooks/N.py` then `python -m jupyter nbconvert --to notebook --execute --inplace notebooks/N.ipynb` — every figure/number re-renders from current data. **The set mirrors the presentation (since 2026-08-07):** **00** = the data and its coverage-robust share estimator; **01** = the episode ground truth (bars 20/40 boom, 12/25 bust, swept); **02** = the feature battery (incl. the §SS August bank extension); **03** = the model tournament (read §SS first — the shipped ensemble selection; the July race is kept above it as provenance); **04** = the deep rules-era evaluation (still the record for the crowd-only detectors); **05** = the influential-users model; **06** = the WORKED EXAMPLE (gold, 2025-26, end to end); **07** = the PRESENTATION PACK — the deck-figure source (`docs/figures/slides/`); **08** = the INFLECTION PROTOTYPE + the PRICE-BLIND study (new gradings, a feature/model search, an AI hover-card prototype, and §8 — the posts-only GET IN/OUT record behind the dashboard's experimental trigger; ~6 min, writes `docs/research/nb08_inflection.json` + `nb08_price_blind.json`). No ordering constraints between them. Retired studies (frozen JSONs still live): `notebooks/_retired_2026-08-07_presentation_refactor/` |
...
| **Is the project still sound?** | `python tools/preflight.py` — 8 checks in a second: configs validate, stores present, every theme has a priced line, no anchor silently substituted, every mapped ticker still exists, frozen constants unmoved, aggregates text-free, no dangling paths. No writes, no network. **Run it after any config edit and before any commit** |
...
- **EUPHORIA: Themes** / **EUPHORIA: Singles** — the headline signal,
  ...
  **Reading the lower panel (rebuilt 2026-07-28).** It now carries ONE
  ...
  80 and inflections over tells you why nothing fired.
