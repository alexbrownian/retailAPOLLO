...
> **ADDENDUM 2026-08-07 — READ BEFORE USING THE NUMBERS BELOW.** This
> brief was written for the July (rules-era) system. The desk signals
> are now scored by a tournament-selected learned model, the ground
> truth was lowered (20/40 boom, 12/25 bust), and the share estimator
> was rebuilt. For the CURRENT numbers and figures use: the comparison
> table `docs/research/ml_tournament.md` (slide version
> `figures/deck/F20_model_tournament.png`), the presentation-pack
> slide figures in `figures/slides/` (merged pack, 2026-08-09; the
> pre-merge set survives in `figures/07/`), the gold worked example in
> `figures/06/`,
> and the data-quality figures in `figures/00/`. Headline replacing the
> July one: GET OUT catches 42% of detectable episode tops (rules: 12%)
> at 0.35 FA/instrument-year, median lead 16 days; GET IN catches 57%
> of starts (rules: 9%) at 0.24. Everything else in this brief (format
> rules, template rules, tone) still applies.
