...
| Switch signal strictness | NOTHING TO RUN — sidebar "signal setting" on the dashboard: **Standard** (F1 cut) or **Strict** (F0.5 cut, ~half the alerts and false alarms). Both cuts frozen walk-forward by the pipeline; both fire on upward CROSSINGS only (2026-08-09, evidence `docs/research/conditioning_sweep.json`) |
