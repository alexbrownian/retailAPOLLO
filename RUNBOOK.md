...
| Rebuild the ONSET detector + DESK signals (GET IN / GET OUT) | `python -m analytics.run_analytics --what phases` — LIVE mode: episode catalog + today's scores/alerts at the frozen thresholds (seconds), including `euphoria_desk.parquet` (scored by the SHIPPED desk model - since 2026-08-07 the tournament-selected logit+GBM ensemble at its frozen probability cuts; the rules path remains the automatic fallback if a frozen record ever names it); add `--research` for the full model tournament + walk-forward scorecards + threshold re-freeze (~15 min - it races 5 model families) |
