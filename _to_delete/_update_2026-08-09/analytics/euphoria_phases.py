...
def alerts_from_scores_crossing(dates: list, scores: list,
                                threshold: float,
                                cooldown: int = EUPHORIA_COOLDOWN_DAYS
                                ) -> list:
    """The CROSSING trigger (desk decision 2026-08-09, evidence
    docs/research/conditioning_sweep.json): fire only on an UPWARD
    crossing of the cut - after a fire, the score must first drop BELOW
    the cut to re-arm (then the usual cooldown still applies). Kills
    the level-trigger's repeated fire every cooldown all the way up a
    long rally: walk-forward, IN/OUT flips within 30d fell 241 -> 183
    with capture within 2 points of the level trigger on both heads."""
    s = np.asarray(scores, dtype=float)
    alerts, last, armed = [], None, True
    for d, v in zip(dates, s):
        if v >= threshold:
            if armed and (last is None or (d - last).days >= cooldown):
                alerts.append(d)
                last = d
            armed = False
        else:
...
def rebuild_phase_files(verbose: bool = True,
    ...
    def _frozen_ml_pair(cand_j, maker, label, mode):
        """(standard F1 cut, strict F0.5 cut) from ONE train scoring."""
        train = cand_j[cand_j["year"] < data_max_year]
        if train.empty:
            train = cand_j
        fit = maker(label)
    ...
    if desk_research:
        ...
        if model_name == "rules":
            ...
            thr_out, thr_out_strict = _frozen_ml_pair(cand_j, maker,
                                                      "y_top", "top")
            thr_in, thr_in_strict = _frozen_ml_pair(cand_j, maker,
                                                    "y_onset", "onset")
        ...
        desk_stored = {
            ...
            "conditioning": {
                ...
                "evidence": "docs/research/conditioning_sweep.json",
                "decided": "2026-08-09",
            },
            "tournament": _tbl,
        ...
        if (model_name != "rules"
            ...
            with open(desk_path, "w") as f:
                _json.dump(desk_stored, f, indent=1, default=str)
            if verbose:
                print(f"  desk record upgraded: strict cuts frozen "
                      f"(in {thr_in_strict:.3f} / out "
                      f"{thr_out_strict:.3f})")
        if verbose:
    ...
    def _alert_dates(scored, thr):
        # CROSSING trigger (desk 2026-08-09) - fire on upward crossings
        # only, so a persistently-hot score cannot re-fire every
        # cooldown all the way up a rally
        by = {}
        for name, g in scored.sort_values("date").groupby("name"):
            by[name] = set(alerts_from_scores_crossing(
                g["date"].tolist(), g["dscore"].tolist(), thr))
        return by

    in_alerts = _alert_dates(onset_scored, thr_in)
    out_alerts = _alert_dates(end_scored, thr_out)
    in_alerts_s = (_alert_dates(onset_scored, thr_in_strict)
                   if thr_in_strict is not None else {})
    out_alerts_s = (_alert_dates(end_scored, thr_out_strict)
    ...
    for _suffix, _ia, _oa in (("", in_alerts, out_alerts),
        ...
        ds[f"get_out{_suffix}"] = [d in _oa.get(n, ())
                                   for n, d in zip(ds["name"],
                                                   ds["date"])]
    # PM-TRUST COHERENCE (kept under every model, one sentence to
    # explain: a START is never shown on a day that already satisfies
    # the ending conditions, and never within one cooldown AFTER an END
    # call - the desk said three times that a START on top of an END is
    # the error that destroys trust. Under the rules model this is a
    # candidacy consequence; under the ML it is applied here, at the
    # display layer, so the invariant holds identically - for BOTH the
    # standard and the strict variant.)
    for _suffix in ("", "_strict"):
        ...
        for _n, _g in ds.groupby("name"):
            ...
            for d in ins:
                if any(0 <= (d - t).days < EUPHORIA_COOLDOWN_DAYS
                       for t in outs):
