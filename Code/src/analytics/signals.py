...
from src.analytics.loaders import (load, to_wide, TICKER_COUNTS,
                               TICKER_COUNTS_BY_SOURCE, TICKER_SENT,
                               THEME_SENT, THEME_SIGNALS, TICKER_SIGNALS)
from src.analytics.conviction import trailing_z
...
def make_decisions(names, all_days, az_f, cz_f, dv_f, crowd_f, xrise_f,
    ...
    for name in names:
        ...
        # otherwise-boolean column, and pandas 2.x deprecates fillna's
        # silent downcast back to bool. Stating the dtype keeps the
        # identical result and avoids the FutureWarning.
