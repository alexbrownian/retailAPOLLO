...
Writes ``Data/abstracted/daily_term_counts.parquet`` (columns ``date``,
``term``, ``mention_count``) from ``posts.parquet``::

    python Code/ingestion/build_term_counts.py
...
``Data/abstracted`` ships it alongside the other aggregates.
"""
