...
* tickers: the ETF's top holdings (Yahoo Finance ``funds_data``), kept
...
* price: pulled through :mod:`src.prices` (Bloomberg or Yahoo Finance)
...
is a deliberate edit (``docs/CONFIGURATION.md``).

Network calls (Yahoo search, holdings, price) are isolated in small
...
class Candidate:
    symbol: str
    name: str
    origin: str          # "approved" (config) or "search" (Yahoo)
...
def search(query: str, limit: int = 8, remote=None,
    ...
    the query itself when it looks like a ticker, then Yahoo Finance
    search. ``remote`` is the search callable (injected in tests); it
    defaults to :func:`_yahoo_search`. When ``status`` is a dict, the
...
# ---------------------------------------------------------------------------
# remote services: Bloomberg first when a Terminal answers, Yahoo Finance
# otherwise (and when the Bloomberg request itself fails)
# ---------------------------------------------------------------------------
...
def _remote_search(query: str, limit: int) -> List[Candidate]:
    """Bloomberg when available (falls through to Yahoo on any error),
    else Yahoo."""
    if bloomberg_available():
        try:
            got = _bloomberg_search(query, limit)
            if got:
                return got
        except Exception:                                  # noqa: BLE001
            pass
    return _yahoo_search(query, limit)


def _yahoo_search(query: str, limit: int) -> List[Candidate]:
    import yfinance as yf
    res = yf.Search(query, max_results=max(limit * 2, 10), news_count=0)
    out = []
    for qte in getattr(res, "quotes", []) or []:
        if (qte.get("quoteType") or "").upper() != "ETF":
            continue
        sym = qte.get("symbol", "")
        name = qte.get("longname") or qte.get("shortname") or sym
        if sym:
            out.append(Candidate(sym, name, "search"))
    return out[:limit]
...
def _clean_ticker(raw: str) -> str:
    """Yahoo holdings symbols: ``BRK-B`` -> ``BRK.B``; ``PETR4.SA`` stays
...
def _remote_holdings(symbol: str) -> List[dict]:
    if bloomberg_available():
        try:
            got = _bloomberg_holdings(symbol)
            if got:
                return got
        except Exception:                                  # noqa: BLE001
            pass
    return _yahoo_holdings(symbol)


def _yahoo_holdings(symbol: str) -> List[dict]:
    import yfinance as yf
    ...
    for tick, r in top.iterrows():
        out.append({"ticker": str(tick), "name": str(r.get("Name", "")),
                    "weight": float(r.get("Holding Percent", 0.0))})
    return out
...
def model_bundle_path() -> str:
    ...
    from analytics.ml_detector import DESK_BUNDLE
...
def model_score(spec: LookupSpec, es, frame: pd.DataFrame, px: pd.DataFrame,
    ...
    """
    from analytics.ml_detector import (attach_price_features, candidate_frame,
                                       load_desk_bundle, score_with_bundle)
    from analytics.euphoria_phases import build_day_frame, boomed120_frame
    bundle = bundle or load_desk_bundle(model_bundle_path())
    if bundle is None:
        return {"status": "no_model",
                "why": "no fitted model on disk yet - run "
                       "`python -m analytics.run_analytics --what phases`"}
...
def _cli(argv=None) -> int:
    ...
    print("model bundle:", "present" if os.path.exists(model_bundle_path())
          else "absent (run analytics.run_analytics --what phases)")
    return 0
