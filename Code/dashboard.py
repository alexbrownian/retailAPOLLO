# dashboard.py
# ============
# The retailAPOLLO dashboard: one Streamlit + Plotly application over the
# whole pipeline. The landing page leads with the names closest to an
# INCREASE EXPOSURE or CUT EXPOSURE call; every chart is hoverable and
# zoomable, and every section leads with a ranked list.
#
#     pip install -r requirements.txt
#     python -m streamlit run Code/dashboard.py
#
# HOW IT IS BUILT
#   * Every number shown here is read from the stores the pipeline wrote
#     (Data/processed, or the published bundle in Data/dashboard on a
#     hosted copy). Heavy computation lives in src/analytics and is shared
#     with the pipeline, so a figure on the page is by construction the
#     figure on disk.
#   * Data loading is cached on (path, file mtime): the page reruns
#     instantly on interaction and refreshes the moment the pipeline
#     rewrites a file.
#   * Branding, the credit and acknowledgement lines, and the pipeline
#     buttons come from config/settings.csv (overridden per machine by
#     config/settings.local.csv).
#
# LAYOUT OF THIS FILE
#   1. constants, CSS, header mark, definition texts
#   2. cached data loading and the hosted-copy bootstrap
#   3. Plotly figure builders
#   4. masthead, sidebar (freshness, pipeline controls)
#   5. topline metric strip
#   6. the views: landing page | AI Pulse | Influence tracker |
...
def _security_names():
    ...
    anchor ETF - a bare list of tickers makes you remember what NBIS or
    SNDK are; a labelled one does not."""
    out = {}
    for fname, col in (("nasdaqlisted.txt", "Symbol"),
                       ("otherlisted.txt", "ACT Symbol")):
        path = os.path.join(DATA_DIR, "reference", fname)
...
# committed bundles are copied into place once per container: Data/abstracted
# (the text-free aggregates) and Data/dashboard (the finished display frames,
# staged by tools/publish_dashboard.py).
#
# Copy, never recompute: src.analytics.run_analytics needs prices, forks a process
...
# ---------------------------------------------------------------------------
BUNDLE_DIR = os.path.join(DATA_DIR, "dashboard")
...
def _bootstrap_from_bundles():
    ...
    for name in sorted(os.listdir(BUNDLE_DIR)):
        ...
        if name == "prices.parquet":
            ...
            _place(src, os.path.join(DATA_DIR, "reference", name))
        elif name == "nb05_influence.json":
            _place(src, os.path.join(DATA_DIR, "research_record", name))
            ...
            for infl in os.listdir(src):
                _place(os.path.join(src, infl),
                       os.path.join(DATA_DIR, "reference", "influence",
                                    infl))
...
def _bundle_signature() -> float:
    ...
    """
    newest = 0.0
    for root in (BUNDLE_DIR, os.path.join(DATA_DIR, "abstracted")):
...
# Pipeline controls (fetch, price pull, comment catch-up) are opt-in via
# config/settings.csv (show_pipeline_controls) or RETAILAPOLLO_CONTROLS=1.
# They call paid APIs and a price terminal, so a public copy keeps them
# off. Read once per process; the settings file is small.
from src import settings as _app_settings  # noqa: E402
LOCAL_CONTROLS = _app_settings.show_pipeline_controls()
APP_TITLE = _app_settings.get("app_title")
APP_TAGLINE = _app_settings.get("app_tagline")
APP_CREDIT = _app_settings.get("app_credit")
APP_THANKS = _app_settings.get("app_thanks")
...
def ai_text(s):
    ...
    $...$ as LaTeX math, so one price mention and a later one would
    swallow whole sentences into italic mush ("$12B ... $100" -> math).
    Escaping the dollar sign is enough - the model's own markdown (bold,
    bullets) still renders."""
...
def flag_label(name, kind):
    ...
    Both sides share the theme-side format: `China geopolitics (FXI)` -
    a name a human recognises, followed by the thing you would actually
    trade.  A bare four-letter ticker such as `IREN` in an INCREASE
    EXPOSURE banner makes the reader stop and remember what IREN, NBIS
    or SNDK are, and the whole point of the banner is that it needs no
    interpretation.

    So both sides use this function, and so does the lookup
    ...
    resolves to its first priced fallback, so `europe_defense` can be
    labelled EUAD while drawn on ITA - a US aerospace line standing in
    for a European one, which is precisely the substitution that theme's
    own config note warns about.  Where they
...
# prices alone. The study of whether it agrees with this project's crowd
# signal is in the research record.
#
...
HIGH_CONV_HELP = (
    ...
    "**The five factors** of the momentum screen. All "
...
# other times it sat here?" - the question a reader actually asks, replacing
...
def level_conditioned_stats(name, level_now, band=10.0, min_n=10):
    ...
    WHY THE EXTRA THREE. Shown alone, the median reads as an argument
    AGAINST the CUT call sitting beside it ("if px is up at this level
    why reduce?"). Two things flip the reading:
      ...
      * NO BASELINE. Equities drift up, so almost any forward median is
        ...
        +3.9% against a +4.4% baseline. The raw figure measures
...
# Themes with show_on_dashboard = no in config/theme_etfs.csv are hidden
# from every list on the page. Display only: their data is still built
# and scored, so the frozen thresholds are unaffected.
from src.themes import HIDDEN_THEMES  # noqa: E402
...
# ---------------------------------------------------------------------------
# DESIGN TOKENS - institutional light theme (adopted
...
# bullish-vs-bearish still has to read in one glance from across a room, so
...
# Injected once; styles an institutional light theme.
INSTITUTIONAL_CSS = f"""
<style>
...
/* --- type ------------------------------------------------------------
   ...
   pipeline host behind a proxy that blocks fonts.googleapis.com still
...
/* --- page ------------------------------------------------------------
   ...
   into three screens of scrolling, which is the one thing a reader will not
...
/* the line under each action head. Larger than a st.caption, which
   renders at the same tiny size as every footnote on the page and would
   bury the one sentence that says what the list IS. */
.rf-actionsub {{
...
# ---------------------------------------------------------------------------
RESEARCH_DIR = os.path.join(DATA_DIR, "research_record")
...
# dashboard carries no copy of it, so the fallback rule cannot drift out
# of step with the engine that actually scores.
...
# ---------------------------------------------------------------------------
# THE EUPHORIA GAUGE: a speedometer per graph - the current euphoria
# percentage, a red zone, and the change.
# ---------------------------------------------------------------------------
def gauge_zones():
...
#
# The card's dial shows readiness, not the 0-100 EUPHORIA level. The two
# are different quantities: the level is a crowd-heat composite, the gap
...
READY_HELP_XP = (
    ...
    "~0.73): treat it as research, not a production signal."
)
...
# the info tooltip rather than on the page.
# THE SEVEN SIGNAL STATES, in one place - the "?" beside the signal state
# names every state, blow-off included. Two families: a FIRED FLAG owns
# the state for its 21-day
...
STATE_HELP = (
    ...
    "- **EXIT WINDOW** — a CUT EXPOSURE fired within 21 days. The reading "
...
def fig_theme_pulse(share, sent, px, symbol, title,
                    ...
                    mood_relative=False):
    """THE POSITION CHART: three STACKED panels on one shared time axis - attention,
...
def _attention_mode_control(key):
    """The attention-line toggle: the LEVEL
...
def _mood_mode_control(key):
    ...
    net-bullish level, which is almost always positive.
...
# ---- IS THIS THE NEWEST DATA?
#
# The masthead prints the date of the newest reading and whether the
# pipeline is behind on it - not `last update: <now>`, which is the
# moment THE PAGE RENDERED and says "just updated" on a dashboard whose
# data stopped three weeks ago.
#
...
with h_left:
    ...
    st.markdown(
        ...
        f'<span class="rf-title">{APP_TITLE}</span></div>'
        f'<div class="rf-standfirst">{APP_TAGLINE}</div>',
    ...
    # Optional credit line from config/settings.csv (app_credit).
    if APP_CREDIT:
        st.markdown(f'<div class="rf-credit">{APP_CREDIT}</div>',
                    unsafe_allow_html=True)
with h_right:
    # The animated mark is shown only on the copy that runs the pipeline:
    # on a hosted copy it reads as an organisation logo, which it is not.
    if LOCAL_CONTROLS:
...
# per-day model component readings (written by the phases step; when a
# bundle lacks the file the hover falls back to the
# observable-ingredient bars)
...
def _ai_pulse_load_cached(mtime):
    from src.analytics.ai_pulse import load as _pl
    return _pl()
...
def _ai_poll_load_cached(mtime):
    from src.analytics.ai_poll import load_series
    return load_series()
...
# copy: on the pipeline host the reader IS the person who can fix it, so
...
if _deploy_behind and not LOCAL_CONTROLS:
    ...
    # the instruction differs by copy: on the pipeline host the reader
...
# HOW MANY CHARTS - why a window can show only a handful, and how the
# set is chosen.
#
...
#   2. IT MUST HAVE ALERTED INSIDE THE WINDOW.  This is deliberate: only
#      the themes / tickers with a euphoria detected in the selected time
#      frame are shown, i.e. no filler names.  It is also what BINDS in a
#      short window - most names have alerted at some point in history,
#      but only a few inside any given few months - so "only 8 charts" is
#      a statement about the window, not about the cap;
#   3. this slider, newest-alert-first.
#
# A low fixed maximum would silently truncate the moment a wide window
# pushed the alerting set past it - and a silent cap reads as "that is all
# there is".  The ceiling is the largest set the filters can possibly
# produce (every theme plus every single that has ever alerted, with
# slack), so the slider can always be opened far enough to see everything,
# and the line printed above the charts states how many qualified versus
# how many are drawn.
how_many = st.sidebar.slider("items per section", 3, 60, 15)

# ---------------------------------------------------------------------------
# Signal configuration (final production values; provenance in
# research.ipynb). None of these is user-selectable: every surface
...
def sig_col(base, frame):
    """The production-signal column for the active trigger + gate setting -
    ...
    (measured: the phase gate blocks ~3/4 of its correct calls);
...
def sig_head(rep, head):
    """The production-record block holding this head's frozen cuts under the
...
STAGES = {
    ...
    "store":    ("Adding the new posts to the data store",
                 ["APPEND:", "folding live raw", "merging live raw",
                  "MERGE:", "live fast path", "hydrated Data/abstracted"]),
...
def pipeline_panel():
    ...
    if p["state"] == "running":
        ...
        # THE LIVE LINE - the newest log line, so slow in-process stages
...
#
# THREE BUTTONS ONLY.  There is no separate "recompute analytics only"
# button - QUICK UPDATE already ends with the same signal recompute, and
# a no-API-only path would save ~60s at the cost of another button to
# explain.  There is no "FULL historical rebuild" button either: it can
# only work on the machine that holds posts.parquet, so elsewhere it
# would be a button that cannot do its job.  From a shell:
# python Code/update_data.py --full
if LOCAL_CONTROLS and st.sidebar.button(
                     "QUICK UPDATE - Bloomberg prices + signals  (~1-3 min)",
                     disabled=_pipe_running,
                     help="No post fetching. Pull prices for the "
    ...
    start_pipeline([(["ingestion/pull_prices.py"], win_env),
...
# Comments ARE part of the full update: this is
...
if LOCAL_CONTROLS and st.sidebar.button(
                     ...
                     help="NOT needed for the ordinary refresh - the full "
                          ...
                          "the board within the production runtime ceiling. Use "
    ...
    start_pipeline([(["ingestion/update_comments.py"], None)],
...
if prices is None and LOCAL_CONTROLS:
    st.sidebar.warning("prices.parquet missing - run pull_prices.py "
                       "or use the rebuild button")
elif prices is None:
    st.sidebar.warning("Price history is not in the published bundle; "
                       "price-linked panels are unavailable.")

# Acknowledgement (config/settings.csv: app_thanks), last thing in the sidebar.
if APP_THANKS:
    st.sidebar.divider()
    st.sidebar.caption(APP_THANKS)
...
if euph is not None and len(euph):
    ...
    # arithmetic as the euphoria tabs' attention sort. Deliberately NOT
...
# ---- One explainer only. The dashboard shows conclusions; the reasoning
# behind each parameter (and the number that settled it) lives in
# research.ipynb and the research record, so a reader who
# challenges a threshold is sent there rather than answered here.
...
# wearing the right clothes, so both properties above hold: the
...
# still tracked in the data - they are simply not shown on the reader.
...
def euphoria_simple():
    ...
    caveat is accurate and unreadable - the reader gets through four
    ...
    baseline, the precision of a red line, or the captured / detectable /
    false-alarm triple.  That is the standing division of labour, not a
    doubt about the figures.  A performance claim needs its interval, its
    window and its judge standing next to it to be read correctly, and none
    of that fits in a fifteen-second explainer; stripped of that scaffolding
    the numbers become slogans a reader repeats without the caveat.  They
    live in `Data/research_record/nb07_performance_battery.json` (the
    index-composite study's frozen record), where the scaffolding is
    present.  What is here is definitional: what euphoria means, what must
    be true before anything can fire, and what this is not - none of which
    moves when the record is re-measured.  Recorded in research.ipynb."""
...
- **Red line = CUT EXPOSURE.** Euphoria is *ending*. Historically the price top
  is close - this is the line to bring to a reader.
...
One table, so the whole rule set is readable in one place instead of
being reconstructed from the register. Every value
...
| A1 | Crowd size vs its OWN normal | 7d mentions >= **{EUPHORIA_ONSET_HYPE_MIN:g}x** its 120d median | 7d mentions >= **{EUPHORIA_HYPE_MULT:g}x** its 120d median | INCREASE EXPOSURE has to fire EARLY, so its bar is lower - but not 1.0x, which breaches the FA budget (0.255 vs {EUPHORIA_FA_BUDGET_PER_IY}) |
...
def euphoria_simple_ml(rep):
    """The plain-language explainer for the LEARNED production model.
    Same fifteen-second budget as euphoria_simple(), but the machinery it
    describes is the model tournament winner, not the gate stack - a
    learned model has fewer hand-set constants to defend, so the
    explainer leads with the three numbers that remain."""
    rep = rep or {}
...
90 days after it. (The bars were set by a three-level sweep - the chosen
definition keeps every reference episode and the model's accuracy
...
def tournament_table_md(rep):
    """The comparison table, read live from the frozen production
    ...
    label = {"rules": "Hand rules (baseline)",
    ...
    out.append(f"*Selection rule (stated before the numbers were "
               ...
               "not comparable across candidacy frames - the baseline's "
...
(a selection rule): price never enters the euphoria level or the
...
**The STARTING line (green)** comes from the onset detector (the
...
**What actually fires on THIS screen - the PRODUCTION CONFIGURATION.**
The production signals use price as well as crowd data, for a better hit
...
removes one-day-blip alerts (AP 0.435 → 0.449, same caveat, two
captures recorded as the cost). The 54d-window record: capture 22/98,
10 FAs (0.083/instrument-year against a 0.23 budget), AP 0.615, 9-day
median warning; CONSIDER sits inside its own budget at 0.200.
...
there is incoherent - so it cannot fire. Measured against the unaware
onset, that cuts start-next-to-end adjacency from 20 to 2 and late
starts from 21 to 10, at a recorded cost of captures (29 → 20 of 125) -
accepted because a START landing on an END destroys coherence, and
documented with the full variant table in the strictness study's frozen
record (nb06_strictness.json).
...
# from the Terminal or typed by hand, words from the ETF's and holdings' names), reads
...
def _lk_bundle(mtime_key):
    ...
    from src.analytics.ml_detector import load_desk_bundle
...
def render_etf_lookup(key_prefix):
    ...
    q = _c1.text_input("ETF name or ticker", key=f"{key_prefix}_lk_q",
                       ...
                       help="Searches the configured instruments first, "
                            "the local ETF catalogue, then the Terminal. Pick a result to build "
    ...
    if _search_err:
        st.caption(f"Terminal search unavailable ({_search_err[:80]}) "
    ...
    if not holds:
        if _hold_err:
            st.caption(f"Holdings could not be fetched from the Terminal "
                       f"({_hold_err[:80]}). Type the main holdings "
                       "yourself to keep going.")
        else:
            st.caption("No holdings source answered for this symbol (no "
                       "Terminal, a commodity or bond fund, or an unknown "
                       "ticker). Type the main holdings yourself.")
    ...
    if _ms["status"] == "ok":
        ...
        _px_thru = (pd.to_datetime(px["date"]).max() if px is not None and len(px)
                    else None)
        _cw_thru = frame["date"].max() if len(frame) else None
        st.caption("Scored on the newest day with both a close and a full "
                   "feature set: price through "
                   + (f"{_px_thru:%d %b}" if _px_thru is not None else "n/a")
                   + ", crowd through "
                   + (f"{_cw_thru:%d %b}" if _cw_thru is not None else "n/a")
                   + ". A close exists only after the market closes, so a "
                     "morning run is always at least one day behind today.")
        if _now["pct"] >= 100:
    ...
    st.caption("Beta. The level and its bands are the same rule-based "
               ...
               "properly, add it as a theme (research.ipynb).")
...
def render_euphoria_tab(kind, kind_label, key_prefix, mode="full"):
    ...
    # HEADING AND LEGEND ARE SEPARATE.  A legend glued onto the subheader
    # wraps on a normal-width screen, and its second line then reads as a
    # heading in its own right - e.g. a red-side phrase sitting directly
    # above the GREEN banner.  A section header contradicting the box
    ...
    # NO EXPLAINER OR REFERENCE EXPANDERS ON THIS TAB. Same reasoning as
    # the page-level block above: the dashboard shows conclusions, and the
    # definition, the full method and the constants register live in
    # research.ipynb. `EUPHORIA_DEF_FULL` (top of this file) is the research
    # text cited by the report; `euphoria_simple()`, `euphoria_simple_ml()`
    # and `tournament_table_md()` are defined and tested, so an on-screen
    # explainer block would be a paste, not a rewrite.

    # ---- ANCHOR TROUBLE, AND ONLY WHEN THERE IS ANY -------------------
    # No reference expanders here; only the CHECK behind them. It catches
    # something real - e.g. `europe_defense` being drawn on ITA, a US
    # aerospace line standing in for a European one, with nothing saying so.
    #
    # Silent by default. Nothing renders while every anchor is priced and
    # in use - so this costs no space on a normal day and speaks only when
    # a theme is quietly being drawn on something other than the
    # instrument it names.
    if kind == "theme":
        _sub, _dark = [], []
    ...
    # `desk_end_fit` / `desk_onset_fit` ignore their `train` argument (the reader
    ...
    from src.analytics.euphoria_phases import (ONSET_BANK, TOP_FEATURES,
    ...
    # EPISODE COHERENCE (a selection rule, asymmetric by
    ...
    # the SIGNAL level by the phase-aware production onset, not by a rule.)
    coherent = {}
    ...
    # One arithmetic, three surfaces (the hover, the per-flag dropdowns
    # and the per-chart "why did it fire" must never disagree): every
    # factor line a human reads is built by the SAME
    ...
    # reader remembers is whichever window happened to be open.  The
    # walk-forward record does not move, and it lives where it can be read
    ...
    # the chart quoting the wrong threshold.
    #
    # The rule: draw the threshold that actually gated the flags being
    # drawn, and plot the series that actually crossed it.  `thr_now` (85)
    # is used ONLY for the fallback path, where the level really is the
    # decider.  No new number is introduced anywhere - both production
    # thresholds are read straight out of euphoria_desk_report.json.
    #
    # No rescaling constants here.  The signal scores are 0-1 and the two
    # rules have different frozen thresholds, so the panel plots
    # score/threshold*100, which puts BOTH rules on a single line at 100.
    # The thresholds are read per-name inside draw_chart as `thr_out_d` /
    # `thr_in_d`, straight out of euphoria_desk_report.json, frozen and
    # unscaled.
    thr_now = None
    ...
    #
    # The scrubber is NOT inside draw_chart: a per-chart slider would give
    # a six-name page six independent sliders.  That is the wrong object:
    # the question a reader asks is "what did the book look like on
    ...
    # the same moment - a comparison error the page itself would create.
    #
    # So there is ONE control, owned by the tab and read by every gauge,
    # every state badge and every 7-day change on the page.  Each chart
    # resolves the date against its OWN level curve (names start and end
    # on different days), but they all resolve the SAME date, so the page
    # is always a single point in time.
    # INFLECTION MARKERS NEED A STORE THAT CARRIES THEM. A parquet written
    # without `inflection` / `inflection_score` makes the panel's guard
    # draw nothing. Silent absence is
    ...
    def draw_chart(name, title_prefix, key):
        ...
        # clipped - see the readiness construction below for
        ...
        # research.ipynb rather than here: a hard-coded hit
        ...
        # ---- THE DISPLAYED STATE, full priority order.  Explainable
        ...
        def _disp_state(_day):
            ...
            # STRICT <, ages 0-20: the badge, the coherence engine and
            # this resolver share one boundary, so at exactly 21 days the
            # banner and this state agree ("no live signal").
        ...
        # No per-chart after-signal outcome record is computed here: the
        # panel shows the level-conditioned stats instead, and the outcome
        # scan (two rolling passes over the full price history, per
        # chart, per rerun) would be the single largest waste on the page.
        _log_scale = False
        if _have_dial:
            ...
            # beside it are two lines each, so a four-column strip would
            # leave the whole right half empty and push the level record
            # into a narrow gutter below. Gauge on the left, and
            # everything else stacked in ONE wide right-hand block that
            # fills the space the gauge's height creates.
            ...
            with _gc:
                ...
                if _ready is not None:
                    ...
                    _g_band = _rd_side
                    # GETIN is this palette's green accent; there is no
                    # TEAL_TEXT token in this file, and a reference to
                    # one would crash every INCREASE-side chart while the
                    # CUT branch sailed past the test.
            ...
            with _rt:
                ...
                with _f1:
                    st.markdown(_facts(_facts_rows),
                                unsafe_allow_html=True)
                with _f2:
                    # Only the one fact a reader acts on. No state
                    # words (calm/quiet), window peak or in/out counts
                    # here - they would describe the chart the reader
                    # is already looking at.
                    st.markdown(_facts([
                        ("last signal", _last_sig, _last_col),
                    ]), unsafe_allow_html=True)
                # CONDITIONED ON TODAY, not on past signals. The median
                # move after this name's own fired signals would be n=1
                # on most names - a story rather than a statistic. This
                # block asks the question a reader actually
        ...
        # ---- READINESS, for the HOVER: hovering the chart shows every
        # factor that contributes to a signal, the percentage of the way
        # to its threshold, and a -100..+100% bar towards the signal
        # firing.
        #
        # There is no readiness panel under the price; the numbers live in
        # the hover instead of on an axis - same crossings, same
        # eligibility, zero panel height.  The readiness series below are
        # the `stored score / frozen threshold` construction,
        # priority-merged with the production scorers' recomputation so
        # the stored value wins on every day the detector actually judged
        # (the full reasoning, and the two rejected drawings, are recorded
        # in research.ipynb 6.13-6.16).
        _ready = []
        if dk_i is not None:
        ...
        # A name the signal store does not COVER must not inherit the
        # level fallback: it would render e.g. "+107% of trigger · could
        # fire today" from the level threshold while its factor lines
        # divide by the production trigger - two arithmetics in one
        # panel.  The fallback exists for a machine with NO signal store
        # at all, nothing else.
        _no_desk = use_desk and (dk_i is None or not len(dk_i))
        if not _ready and thr_now and not use_desk:
            # FALLBACK: with no signal store the euphoria level really is
            # the decider, so the identical construction applies to it.
            _ready.append(("SIGNAL", lvl / float(thr_now) * 100.0))
        _merged = []
        # the rules-score EXTENSION below back-fills days the signal store
        # does not cover by recomputing the RULES arithmetic live. That
        # is only coherent when the rules ARE the shipped model - under
        # the learned model a rules score divided by the
        # model's probability cut would be two arithmetics in one panel,
        # so the extension is disabled and uncovered days simply show
        # nothing.
        ...
        # ONE builder, two consumers: compact (_full=False) feeds the
        # HOVER - a three-line glance that does not bury the price - and
        # full (_full=True) feeds the WHY panel rendered UNDER the chart
        # so the reasons sit outside the graph.  Same
        ...
        def _day_lines(_i2, _full=False):
            ...
            if _no_desk and _on_mean is None:
                ...
                # above real factor values - say the truth instead.
                _ph = ("UNTRACKED", INK_MUTED,
                       "the production detector does not cover this name (too "
        ...
        # ---- ONE PANEL: THE PRICE.  No euphoria chart below it -
        # everything a lower panel would say is in the hover above; the
        # flags are vertical rules + dots on the price line itself.
        fig = go.Figure()
        ...
        # Not parked at `level.min()`: that is a EUPHORIA value (0-100)
        # on an axis that is usually PRICE, so every diamond would land
        # near y=0, detached from the series, looking like a broken glyph
        # on the axis. `_carrier` is whatever line is actually drawn
        # (price when there is price, the level curve when there is not),
        # so this cannot go wrong when the axis changes underneath it.
        if inflection_alerts:
            ...
            # end of that shorter series (IndexError)
        ...
        # alternating two fixed heights would collide at i and i+2 on
        # long windows).
        ...
        # unset title into the literal string "undefined" (observed in
        # the live DOM).  Keep it.
        ...
        if (dk_i is not None and len(dk_i) and IN_SCORE in dk_i.columns
            ...
            if len(_srg):
                ...
                # ungated (measured: the gate blocks ~3/4 of its
                ...
                # FIRE-DAY OVERRIDE. Without it a fired INCREASE marker
                # could sit over the CUT band. The gate
            ...
            if len(_srg) and len(_sr):
                ...
                _BUCKETS = (
                    ...
                    # labelled "momentum", not "price": the bucket is the
                    # run-up + the 1-month return, i.e. momentum readings
                    ("momentum", "#5b6673",
    ...
    if mode == "action":
        ...
        # side, the one the model leads with today (boomed -> CUT, else
        # INCREASE). Scoring every name on BOTH sides would list it
        # twice, putting the same theme under INCREASE and CUT at once:
        # not two readings, one contradiction. NB:
        ...
        def _near_since(scored, col, thr):
            ...
            signal store's integer index, so positional index values would
        ...
        # SHARE OF DISCUSSION. For each name, its mentions over the
        # trailing ROLL days as a share of all mentions of the same kind
        # (themes against themes, single names against single names),
        # and its rank within that kind (1 = most discussed). Shown
        # beside the readiness so a reader sees both how close a name
        # is to a signal and how much of the conversation it owns.
        def _discussion_share(as_of):
            out = {}
            lo_d = pd.Timestamp(as_of) - pd.Timedelta(days=ROLL)
            for _frame, _key in ((theme_counts, "theme"),
                                 (ticker_counts, "ticker")):
                if _frame is None or not len(_frame):
                    continue
                _w = _frame[(_frame["date"] > lo_d)
                            & (_frame["date"] <= pd.Timestamp(as_of))]
                if _key == "theme":
                    _w = _w[~_w["theme"].isin(HIDDEN_THEMES)]
                _tot = _w.groupby(_key)["mention_count"].sum()
                _tot = _tot[_tot > 0].sort_values(ascending=False)
                _all = float(_tot.sum())
                if _all <= 0:
                    continue
                for _rank, (_nm, _cnt) in enumerate(_tot.items(), start=1):
                    out[_nm] = (100.0 * float(_cnt) / _all, _rank,
                                len(_tot))
            return out

        _a_share = (_discussion_share(_a_as_of)
                    if _a_as_of is not None else {})
        ...
        if dk is not None and len(dk) and _a_as_of is not None:
            for _n, _g in dk[dk["date"] <= _a_as_of].groupby("name"):
                ...
                _a_rows.append({
                    ...
                    "side": _side,
                    "share": _sh[0] if _sh else None,
                    "share_rank": _sh[1] if _sh else None,
                    "share_of": _sh[2] if _sh else None,
        ...
        def _a_row(r, side, tone, key):
            rid = f"{side}:{r.name}"
            _share_txt = ""
            if getattr(r, "share", None) is not None and pd.notna(r.share):
                _pct = (f"{r.share:.1f}" if r.share < 1.0
                        else f"{r.share:.0f}")
                _share_txt = (f"   ·   {_pct}% of social media "
                              f"discussion (#{int(r.share_rank)})")
            ...
            if r.hiconv:
                ...
                # factors also correlate" one. The (?) rides ON the
                # banner - st.markdown places its help icon at the end
                # of the text.
                st.markdown(
                    ...
                    "on this name today — <b>five-factor momentum "
                    "screen</b>, detail behind the (?)</div>",
            ...
            for _f in _fired[:ACTION_FIRED_ROWS]:
                ...
                if _hc_f:
                    ...
                    st.markdown(
                        ...
                        "signal on the day it fired — <b>five-factor "
                        "momentum screen</b>, detail behind "
                        "the (?)</div>",
                        unsafe_allow_html=True, help=HIGH_CONV_HELP)
                draw_chart(_f["name"], "",
    ...
    # Themes are shown WITH their tradeable anchor ETF - the instrument
    # comes straight from config/theme_etfs.csv
    ...
    #     alert inside the window are charted, newest first.
    ...
    if sort_mode.startswith("closest"):
        # THE WATCHLIST: which names are closest / eligible to fire, how
        # close, and in what direction - the ones to be watching.
        #
        ...
        for _n, _g in dk[dk["date"] <= _as_of].groupby("name"):
            ...
            # INFLECTION joins the watchlist as a third SIDE (ranked by
            # proximity to an inflection). It is always eligible - the
            # inflection head has no
        ...
        if not show:
            ...
            # so a window with few graphs explains itself - the
            # denominator is names WATCHABLE in the window, and names
            # whose history ends before it are counted out loud with the
            # fix attached.
    ...
    # lists; their scoring fields go unread here.
...
# There is no single-name tab on the display. render_euphoria_tab
# still takes kind="single" and is exercised by the tests, so showing
# single names would be a one-line change.

# ---- ETF RADAR: a table of every ETF tracked, its current retail
# attention score and its graph over time, ordered by what is closest
# to a CUT EXPOSURE ----
...
# The radar is not its own tab: it is the "show the full list" panel
# underneath the landing page, driven by the toggle the action view
...
colour for a reader - *"the accounts with a track record are leaning short semis
...
It is background colour for a reader - *"the accounts with a record are leaning
...
because they were right 100% of the time. Printed as "usefulness 0.987" it
would read as an accuracy, and it is not one: it is a position in a field.
Every exhibit here therefore displays
...
`src/analytics/influence_graph.py::influence_index`, so no chart can drift into a
different scale.

**The same problem in `weighted_voices`, and why it is shown as a SHARE and
not a multiple** (measured). `weighted_voices` is a unitless sum of
(influence x conviction) per name, so it needs a reference before it can be
spoken. Dividing it by the **median name** in the same window - "2.4x the
typical name" - looks like the euphoria detector's A1 convention ("2x its own
120d median"), but **A1 is not analogous.** A1 divides a name by *its own*
history, which is a stable reference. The median NAME divides by whatever
sits in the middle of a long-tailed cross-section, and in this store the
middle is a ticker one person mentioned once: in a measured week 163 names
were mentioned and the median one carried 0.24 of backing, so the top name
would print **141x**. Weekly maxima of 141x, 41x, 2.7x, 14x and 26x on the
30-day view and 171x on the 90-day view are numbers no one can say out loud,
and they move with how many one-off tickers a given fetch happened to catch
rather than with the crowd. With an empty author list the median is exactly
0 and every ratio is NaN.
...
the top quartile of reply-graph PageRank but below median influence. They are the accounts a *follow-the-big-names* strategy would copy,
and the evidence says fade them.

**The time panels** add the dimension the snapshot exhibits lack.
`calls.parquet` carries a date on every call; a snapshot pooling the whole
window can say "GME is the most-backed name" but never "GME's backing has
tripled in three weeks", which is the statement a reader can act on. Both
panels are weekly,
not daily: the comment fetch runs about twice a week under the ingestion
budget (research.ipynb), so a daily axis would largely plot the
...
_INFL_DIR = os.path.join(DATA_DIR, "reference", "influence")
...
def fig_influence_map(nodes: pd.DataFrame, links: pd.DataFrame, title: str,
    ...
    # reports that centrality and being right are near-unrelated, so a hit
    # rate on a node would invite exactly the inference the number refutes.
...
# No `_rgba(hex, alpha)` helper here: every chart states its weight on
# an axis instead of in an alpha channel, which is the readable way
# round and needs no colour arithmetic.
...
def fig_influence_bubbles(dig: pd.DataFrame, voices: pd.DataFrame,
    ...
    called, bullish - which is precisely the crowding a reader is asked to watch.
    ...
    The vertical axis is NOT raw `weighted_voices`: a sum of (score x
    conviction) has no unit, so "influence behind it 3.42" tells a reader
    nothing and cannot be compared between two windows.  `ig.backing_share`
    makes it a per cent of everything the panel said in the window (see the
    block comment above that function for the measured reason share beats
    "x the median name").  It is a positive rescaling, so the ORDER of the
    bubbles is the same as the raw sum's - only readable aloud.
    ...
    # IS TAKEN.  Given an already-truncated frame the denominator would be
    # the 30 names being drawn rather than the 51 in the window: a name
    # would print 29.5% on the chart while the KPI above it printed 26.1%
    # for the same window, and the even-split line would read 100/30 =
    # 3.3% instead of 100/51 = 2.0%.  One quantity cannot have two values
    # on one screen, so the caller passes the FULL digest plus how many
    # bubbles to draw, and "share of the room" always means the whole
...
# ---------------------------------------------------------------------------
# THE TWO SUPPORTING CHARTS
#
# There is no consensus bar chart: a horizontal bar of the same per-ticker
# consensus the bubble chart plots on its x-axis, with the y-axis folded
# into bar OPACITY, is one exhibit and a distraction - opacity is the
# least readable visual channel there is.
#
# The two charts below use the dimensions the bubble chart does NOT have:
#   * fig_crowding_time  - TIME.  The store carries call dates.  "Crowded"
#     is a snapshot; "crowding, and building for three weeks" is a decision.
...
#
# The exact per-ticker numbers are on the tab, in the table beside these
# charts.  Recorded in research.ipynb.
# ---------------------------------------------------------------------------
def fig_crowding_time(tilt: pd.DataFrame, hist: pd.DataFrame, title: str):
    ...
    # the same share do not print their tickers through each other (two
    # names ending a 30-day view at ~0.3% would otherwise render as one
    # illegible smear).  There is no legend precisely so the lines are
    # identifiable by name, so losing a name to a collision would defeat
    # the point.  Drawn as ANNOTATIONS rather
...
def fig_ticker_backers(bk: pd.DataFrame, ticker: str, title: str):
    ...
    the store), NOT an accuracy: see src/analytics/influence_graph.py's
...
if active_tab == "Influence tracker":
    ...
    if not os.path.exists(_INFL_SCORES) and not LOCAL_CONTROLS:
        ...
        st.info("no influence store on this machine yet - it builds "
                ...
                "(`python Code/update_data.py`, or the sidebar button) and "
        ...
        # panel, and that distinction is measured rather than assumed. Cut
        # the top 25 into weeks and a week can hold a single call: one
        # name, which is 100% of that week's conviction by definition, and
        # a "share of the room" built from four people is not a fact about
        # the room. Over all recorded voices the same weeks hold hundreds
        # of calls across dozens of names, which a weekly statistic can
        # stand on. The panel slider exists to keep the NAME-level exhibits
        # legible; it does not define the population whose mood is
        # measured.
        ...
        # The five headline numbers. Not among them: a "HIGH tier" count
        # (a threshold nobody on this tab can act on) or a "crowd base
        # rate" (an ACCURACY figure with no per-author hit-rate column to
        # be read against). In their place is the tab's time dimension:
        # whether the crowding is building or fading. The HIGH-tier count
        # appears in the metric help text.
        ...
        # SUB-TABS, NOT ONE LONG SCROLL.  Sections stacked vertically run
        # to several screens, so the reader scrolls past exhibits to reach
        # the one they want and the shared controls scroll out of sight.
        # Each sub-tab is about one screen, and the panel size / recency
        # controls stay pinned above them so they govern every sub-tab.
        # Grouping is by QUESTION, not by section number.
        # THE MAP LEADS. It is the exhibit that answers the tab's own
        # question at a glance - who is talking to whom. There is no
        # leaderboard / per-name breakdown (a directory, not a decision)
        # and no "what they are pushing" bubble chart (it did not carry
        # its weight). Two views: who talks to whom, and whether the
        # crowding is building.
        _i4, _i2 = st.tabs(["The map", "Building or fading?"])
        with _i2:
            # ---- 2. IS IT BUILDING OR FADING? -----------
            # The tab's one time axis: without it every reading here is a
            # still photograph. The store holds call dates, so the trend
            # is plotted from them.
        ...
        with _i4:
            ...
            # Both boards are this tab's headline finding and it rests on
                    ...
                    st.dataframe(
                        _lw[["author", "influence", "followers",
                             "latest_calls"]].rename(columns={
                                 ...
                                 "latest_calls": "latest calls"}),
                        width="stretch", hide_index=True)
                    st.caption("The accounts a 'follow the big names' strategy "
...
if active_tab == "Top trends":
    st.subheader("Most-mentioned themes (rank 1 = top trending)",
        ...
        "tab."))
    st.caption("Each chart is the position view: the anchor "
...
if active_tab == "Emerging trends":
    ...
    if not movers:
        ...
        st.dataframe(mv.assign(theme=mv["theme"].map(theme_label)),
                     width="content", hide_index=True)
        st.caption("Charts show the position view: price + "
...
def _market_read(day_ts, tc_m, ts_m, tk_m):
    ...
    Drives the slider at the top of the page: what the market is
    feeling on any chosen point in time.

    WHY THIS IS NOT THE LLM. The prose on this page needs a gateway call
    that takes ~30s and only runs on the pipeline host, so it cannot move
    ...
    if ts is not None and len(ts):
        ...
        if len(r) and r["n_posts"].sum():
            nb = float((r["net_bullish"] * r["n_posts"]).sum()
                       / r["n_posts"].sum())
            out["net_bullish"] = nb
            # NO BULLISHNESS SCORE: the weekly net-bullish figure is not
            # ranked against its own trailing history or printed as a
            # 0-100 score. The raw net_bullish is available to the
            # snapshot text, and breadth (below) is the participation
            # read.
...
def _breadth_clause(br):
    ...
    the sentence has to say wide-or-narrow, not loud-or-quiet. (There is
    no intensity percentile or bullishness score to pair it with; see
    research.ipynb.)"""
    if br is None:
        return None
    if br >= 0.7:
...
def _snapshot_text(mr):
    ...
    a ~30s gateway call. But the narrative a reader actually wants from a
...
if active_tab == "AI Pulse":
    ...
    # ~30s gateway call that only runs on the pipeline host, so they
    ...
    if _mr_lo and _mr_hi and _mr_lo < _mr_hi:
        ...
        if os.path.exists(_pp):
            ...
            # the pipeline host. But a page showing 12 March at the top
    ...
    with _c2:
        if LOCAL_CONTROLS and st.button(
                     ...
                     help="Re-runs BOTH sections: the retail-prompt "
                          ...
                          "calls). Needs a configured AI provider. Both also run "
                          "automatically at the end of every update."):
            start_pipeline([(["-m", "src.analytics.ai_poll"], None),
                            (["-m", "src.analytics.ai_pulse"], None)],
    ...
    with _c1:
        if _pulse_real:
            ...
            st.warning("No pulse generated yet - the sections below are "
                       ...
                       "update (or the button here) on the pipeline host "
                       "to fill them from your data.")

    if _pulse_real:
        # the LLM's X/100 fear-greed metric is not shown - the vibe
        # bullets and one-liner
        # carry the mood read; no numeric score is printed or requested
        # from the model (see src/analytics/ai_pulse.py, same date).
        ...
        if _tb:
            ...
            _b = _labs[_pick]
            _th = _b.get("theme", "?")
            # Two metrics only; conviction has no display surface here.
        ...
        # gateway, so its numbers are the same ones the dashboard shows
        ...
        # generated on this machine. A grey caption under a
        # working-looking dropdown would read as "the product covers
        # four themes". Say it loudly, say how many there really are,
        # and say what to run.
    ...
    if _pl is None or not len(_pl):
        st.info("No poll runs yet. The poll runs automatically at the "
                "end of every update on the pipeline host (Apollo "
    ...
    # claim is that every sentence is auditable against stored evidence
    # is the one thing that should not be there.
    # There is one time control, at the top, that owns the date - the mood
    ...
    # from src/analytics/ai_pulse.py, so it cannot drift from what was
    # actually sent.
    with st.expander("the exact prompt behind this page"):
        try:
            from src.analytics import ai_pulse as _apm
...
if active_tab == "[dev] Data Stats":
    ...
    st.caption("Everything on this tab is read live from the stores and "
               ...
               "files' own mtimes.")
    render_how_it_works(desk_report)
    st.divider()
    ...
    _stores = [
        ...
        ("production signals (INCREASE EXPOSURE/OUT)", "euphoria_desk.parquet"),
    ...
    _ing_rows = []
    _ref = os.path.join(DATA_DIR, "reference")
    ...
    st.dataframe(pd.DataFrame(_ing_rows), hide_index=True,
                 width="stretch")
    _raw_root = os.path.join(DATA_DIR, "raw")
    ...
    st.caption(f"Production model: **{_mdl_ds}** | Standard cuts (F1): INCREASE EXPOSURE "
               ...
               "research.ipynb, "
               "Data/research_record/alert_shape_sweep.json")
    if _xp_ds:
        ...
        st.caption(f"Experimental price-blind trigger: **{_xp_ds.get('model')}** "
                   ...
                   "record: Data/research_record/nb08_price_blind.json")
