...
| Read the TURN markers | NOTHING TO RUN — `python update_data.py` writes `turn` / `turn_score` into `euphoria_desk.parquet` every run and the price panel draws them as small diamonds on the axis. **A TURN is CONTEXT, not a call**: it means a reversal is more likely than usual in the next two weeks, it fires at tops AND bottoms with no direction, and it hits 9.6% of the time against a 5.6% base. Use it to corroborate — a GET OUT with a TURN beside it is a more interesting GET OUT — never on its own. In the watchlist it is a **column** — how close each name is to a turn — not a competing sort: the ranking stays on the two real calls, because context should not decide what you look at first. It still cannot fire, gate or re-score a call (fenced by `TestTurnMarker`). **The signal setting was renamed 2026-08-12**: what was *Strict* is now **Standard** and is the DEFAULT; what was *Standard* is now **Relaxed**. The stored columns are unchanged — `*_strict` is the F0.5 cut that Standard now serves, bare columns are the F1 cut that Relaxed serves. Threshold frozen in `euphoria_desk_report.json` under `turn`; re-derive with `--research`. Evidence: `docs/research/turn_trigger_sweep.json`, prototype in notebook 08. **Known weakness:** it beat its base rate in 4 of the 6 years with data and failed in 2022 and 2023 |
