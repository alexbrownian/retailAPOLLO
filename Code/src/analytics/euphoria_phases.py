...
This module extends the top detector in src/analytics/euphoria.py (which calls
...
6. The GET IN / GET OUT pair - the deployed signal family (rule-based
   ...
   learned families live in src/analytics/ml_detector.py).
...
# Data/research_record/nb02_feature_stats.json): source_breadth is
...
# ---------------------------------------------------------------------------
# Trigger swept in Data/research_record/inflection_trigger_sweep.json;
...
# improved the inflection head (Data/research_record/nb08_inflection.json).
...
def inflection_label_frame(series: list, pxmap: dict) -> pd.DataFrame:
    ...
    evaluation (Data/research_record/max_performance.json).
...
#    (Data/research_record/nb03_tournament.json) is the RULES model -
...
def rebuild_phase_files(verbose: bool = True,
    ...
    from src.analytics.loaders import load as _load, THEME_COUNTS, THEME_SENT, \
        ...
        if research and verbose:
            ...
            if _lag is not None:
                print(f"  NOTE: onset data reaches {data_max_year}; the "
                      ...
                      "src.analytics.run_analytics --what phases --research")
    ...
    # runs the full model tournament (src/analytics/ml_detector.py: the
    ...
    from src.analytics import ml_detector as mld    # local: avoids cycle
    ...
    if model_name == "rules":
        ...
        # Data/research_record/nb08_price_blind.json). Everything
    ...
    # Data/research_record/alert_shape_sweep.json): phase gates from the
    ...
    # Data/research_record/nb08_single_dial.json). The 120d phase
    ...
    if inflection_scored is not None and _ttrain_sc is not None:
        ...
        if isinstance(desk_stored, dict) and _infl_src != "frozen":
            ...
            desk_stored["inflection"] = {
                ...
                "bank": _tbank,
                "evidence": "Data/research_record/inflection_trigger_sweep.json",
    ...
    if xp_in_scored is not None and xp_train is not None:
        ...
        if research or "get_in" not in _frozen_xp:
            _xp_rec = {"model": "logit", "bank": _xp_bank,
                       ...
                       "evidence": "Data/research_record/nb08_price_blind.json",
    ...
    # Data/research_record/nb08_retail_flow.json). Failure-isolated:
...
#    measured upgrades (Data/research_record/nb06_desk_config.json):
...
#        24). One loud afternoon cannot fire a one-day episode.
#
