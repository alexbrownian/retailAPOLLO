...
def verify_abstracted(fh):
    """Confirm Data/abstracted is present, small, and carries no columns
...
def main():
    ...
    def _git_autopush():
        ...
        _paths = [os.path.join(DATA_DIR, "dashboard"),
                  os.path.join(DATA_DIR, "abstracted")]
    ...
    if not dry:
        ...
        # The fold's own verdict, separate from the text-free check, so a
        # crashed fold leaves a trace in this summary.
        if fold_failed:
        ...
        log(f"  dashboard     : python -m streamlit run Code/dashboard.py", fh)
        log("=" * 60, fh)
    # A failed fold is a failed run: both the text-free check and the fold
    # set a nonzero exit, so a scheduler or wrapper never sees a crashed
    # fold as success.
