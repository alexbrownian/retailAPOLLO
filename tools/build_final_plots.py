...
# =====================================================================
# 13 — one post, end to end: raw text -> abstraction -> ticker -> mood
# =====================================================================
# NEW 2026-08-12 (desk: "what the raw data looks like (e.g. a sample gold
# post) / the abstraction - data privacy and stuff / the ticker
# extraction - eg. what we use to detect a ticker / the finvader
# extraction").
#
# Everything on this figure is COMPUTED, not typed: the ticker list, the
# theme tag, the lexicon layer sizes, which words fired and at what
# valence, and the compound score all come from the shipped functions at
# render time. If a guard list grows or the lexicon changes, the figure
# changes with it - a hand-typed diagram would quietly go stale, and this
# is the one slide whose whole claim is "this is what actually happens".
#
# THE POST IS REAL, and shown clipped and unattributed - the same
# precedent notebooks 00 and 06 set. That is the point of panel 2: the
# words exist at ingest, in data/raw/ (gitignored, desk machine only),
# and are destroyed at the abstraction step. Nothing that identifies a
# person appears here or in any committed store.
print("13   one post end to end ...", flush=True)
from src.sentiment import (get_analyzer, score_text,               # noqa: E402
                           WSB_LEXICON, _finvader_lexicons,
                           BULL_CUTOFF, TRUNCATE_CHARS)
from src.extract_tickers import (extract_tickers_from_text,        # noqa: E402
                                 CASHTAG, WORD_BARE, STOP_TICKERS,
                                 BARE_PROSE_STOP, ALLOW_TICKERS,
                                 SCREENED_STOP)
from src.themes import themes_in_text as _themes_in_text           # noqa: E402
import re as _re13                                                 # noqa: E402

POST = ("I'm personally bullish on gold ($GLD) right now. I think we've "
        "reach the bottom. There will be some resistance on the way up "
        "but seems like a solid buy.")
POST_SUB, POST_DAY = "r/wallstreetbets", "2026-07-31"
_univ = set(prices["symbol"].unique())
_tk = extract_tickers_from_text(POST, _univ, cashtags_only=False)
_th = sorted(_themes_in_text(POST))
_score = score_text(POST)
_an = get_analyzer()
_fin = _finvader_lexicons()
_lex = _an.lexicon
_fired = [(w, _lex[w.lower()])
          for w in _re13.findall(r"[A-Za-z']+", POST)
          if w.lower() in _lex and abs(_lex[w.lower()]) > 0.001]


def _layer_of(w):
    wl = w.lower()
    if wl in WSB_LEXICON:
        return "WSB slang"
    return "FinVADER" if wl in _fin else "VADER"


_LAYER_COL = {"WSB slang": GREEN, "FinVADER": BLUE, "VADER": GREY}
...
def _panel(ax, n, title, subtitle, col):
    ax.set_xlim(0, 100)
    ax.set_ylim(0, 100)
    ax.axis("off")
    ax.add_patch(mpatches.FancyBboxPatch(
        (1, 1), 98, 98, boxstyle="round,pad=0,rounding_size=2",
        facecolor=PALE, edgecolor=LIGHT, lw=1.4))
    ax.add_patch(mpatches.Circle((6.2, 92), 3.1, color=col, zorder=3))
    ax.text(6.2, 92, str(n), ha="center", va="center", color="white",
            fontsize=13, fontweight="bold", zorder=4)
    ax.text(11.5, 92.6, title, fontsize=14.5, fontweight="bold",
            color=col, va="center")
    ax.text(11.5, 87.0, subtitle, fontsize=11, color=GREY, va="center")


# ---- 1. the raw post -------------------------------------------------
ax = fig.add_subplot(gs[0, 0])
_panel(ax, 1, "What the raw data actually is",
       "one post, exactly as it arrives from the forum", NAVY)
ax.add_patch(mpatches.FancyBboxPatch(
    (6, 40), 88, 36, boxstyle="round,pad=0,rounding_size=2",
    facecolor="white", edgecolor=SKY, lw=1.6))
ax.text(10, 71, f"{POST_SUB}   ·   {POST_DAY}   ·   score 41",
        fontsize=10.5, color=GREY, va="center")
_wrapped = POST
_lines, _cur = [], ""
for _w in _wrapped.split():
    if len(_cur) + len(_w) + 1 > 52:
        _lines.append(_cur)
        _cur = _w
    else:
        _cur = (_cur + " " + _w).strip()
...
for _i, _ln in enumerate(_lines):
    ax.text(10, 64.5 - _i * 5.4, _ln.replace("$", r"\$"), fontsize=12.5,
            color=NAVY, va="center", style="italic")
ax.text(6, 31, f"{len(POST)} characters of somebody's opinion. No price, "
               f"no fundamentals,\nno structure — this is the entire input.",
        fontsize=11.5, color=NAVY, va="top")
ax.text(6, 18.5, "It lives in  data/raw/  on the desk machine only.\n"
                 "That folder is gitignored and is never committed.",
        fontsize=11, color=GREY, va="top")

# ---- 2. the abstraction ---------------------------------------------
ax = fig.add_subplot(gs[0, 1])
_panel(ax, 2, "What we keep, and what we destroy",
       "the text-free boundary, enforced in code", GREEN)
ax.add_patch(mpatches.FancyBboxPatch(
    (5, 42), 43, 36, boxstyle="round,pad=0,rounding_size=2",
    facecolor="white", edgecolor=GREY, lw=1.4))
ax.text(26.5, 74, "THROWN AWAY", ha="center", fontsize=11.5,
        fontweight="bold", color=GREY)
for _i, _t in enumerate(["the words themselves", "the username",
                         "the post id / permalink",
                         "any link back to a person"]):
    ax.text(9, 67 - _i * 6.0, f"✕   {_t}", fontsize=11.5, color=GREY,
            va="center")
ax.add_patch(mpatches.FancyBboxPatch(
    (52, 42), 43, 36, boxstyle="round,pad=0,rounding_size=2",
    facecolor="white", edgecolor=GREEN, lw=1.8))
ax.text(73.5, 74, "KEPT", ha="center", fontsize=11.5, fontweight="bold",
        color=GREEN)
for _i, _t in enumerate([f"the date          {POST_DAY}",
                         f"the ticker        {', '.join(_tk)}",
                         f"the theme         {', '.join(_th)}",
                         f"one number        {_score:+.2f}"]):
    ax.text(56, 67 - _i * 6.0, _t, fontsize=11.5, color=NAVY,
            va="center", family="monospace")
ax.text(5, 34, "That is the whole row. Four fields, no text — and the "
               "post can never\nbe reconstructed from them, because the "
               "words are gone before\nanything is written.",
        fontsize=11.5, color=NAVY, va="top")
ax.text(5, 18, "Not a policy — a WRITE-TIME CHECK refuses to save a store "
               "containing\ntext, and a unit test fails the build if the "
               "check is removed.",
        fontsize=11, color=GREEN, va="top")

# ---- 3. finding the ticker ------------------------------------------
ax = fig.add_subplot(gs[1, 0])
_panel(ax, 3, "How a ticker is found",
       "two patterns, then four guard lists", BLUE)
ax.text(6, 79, "PATTERN", fontsize=10.5, fontweight="bold", color=GREY)
ax.text(6, 72.5, r"$([A-Z]{1,5})\b", fontsize=13, color=NAVY,
        family="monospace")
ax.text(41, 72.5, "a cashtag  →  $GLD ✓", fontsize=11.5, color=GREEN)
ax.text(6, 65.5, r"\b([A-Z]{4,5})\b", fontsize=13, color=NAVY,
        family="monospace")
ax.text(41, 65.5, "bare CAPS, original case", fontsize=11.5, color=NAVY)
ax.text(6, 57.5, "Scanned on the ORIGINAL text, never an uppercased copy: "
                 "only words the\nposter really shouted can be bare "
                 "tickers. Lowercase \"gold\" here is prose;\n\"GOLD\" "
                 "would have been Barrick.",
        fontsize=11, color=GREY, va="top")
ax.text(6, 40, "THEN FOUR GUARD LISTS", fontsize=10.5, fontweight="bold",
        color=GREY)
_guards = [(f"{len(STOP_TICKERS)}", "jargon that is never a ticker (CEO, "
            "ATH, YOLO)"),
           (f"{len(BARE_PROSE_STOP)}", "English words in disguise (ALL, "
            "OPEN, REAL)"),
           (f"{len(SCREENED_STOP)}", "cashtag-only names — $ACTS counts, "
            "\"ACTS\" does not"),
           (f"{len([t for t in ALLOW_TICKERS if len(t) < 4])}",
            "short names let back IN (MU, AMD, GLD, KO)")]
for _i, (_n, _t) in enumerate(_guards):
    ax.text(9, 33 - _i * 6.2, _n, fontsize=13, fontweight="bold",
            color=BLUE, ha="right", va="center")
    ax.text(12, 33 - _i * 6.2, _t, fontsize=11.3, color=NAVY, va="center")
ax.text(6, 6.5, "Rejected here: \\$7500 (not letters) · \\$BTCUSD "
                "(six) · lowercase 'gold' (prose)",
        fontsize=10.8, color=GREY)

# ---- 4. the mood score ----------------------------------------------
ax = fig.add_subplot(gs[1, 1])
_panel(ax, 4, "How the mood is scored",
       "FinVADER — a finance-tuned lexicon, not a black box", BLUE)
# If finvader is not installed the engine degrades to plain VADER by
# design - in that case say so on the figure rather than drawing an
# empty FinVADER band and captioning it as if it were there.
_layers = [("VADER base", len(_lex) - len(_fin) - len(WSB_LEXICON), GREY),
           ("+ FinVADER finance terms", len(_fin), BLUE),
           ("+ WSB slang, hand-set", len(WSB_LEXICON), GREEN)]
_layers = [L for L in _layers if L[1] > 0]
if not _fin:
    ax.text(7, 86, "finvader not installed on this machine - rendered "
                   "from plain VADER", fontsize=9.5, color="#B4553F")
_x = 7
for _nm, _n, _c in _layers:
    _w = 84 * _n / max(len(_lex), 1)
    ax.add_patch(mpatches.Rectangle((_x, 70), _w, 6.5, color=_c))
    _x += _w
ax.text(7, 80.5, f"{len(_lex):,} scored terms, in three layers "
                 f"(later layers win)", fontsize=11, color=NAVY)
for _i, (_nm, _n, _c) in enumerate(_layers):
    _lx = 7 + _i * (86 / max(len(_layers), 1))
    ax.text(_lx, 64, f"■ {_nm}", fontsize=10, color=_c)
    ax.text(_lx, 59.5, f"   {_n:,}", fontsize=10.5, color=GREY)
ax.text(7, 52, "WORDS THIS POST FIRED", fontsize=10.5, fontweight="bold",
        color=GREY)
_show = [f for f in _fired if abs(f[1]) >= 0.15][:8]
for _i, (_w, _v) in enumerate(_show):
    _col = _i % 2
    _row = _i // 2
    _bx, _by = 8 + _col * 45, 44.5 - _row * 7.2
    ax.text(_bx, _by, _w, fontsize=12, color=NAVY, va="center",
            fontweight="bold")
    ax.text(_bx + 22, _by, f"{_v:+.2f}", fontsize=11.5, va="center",
            color=(GREEN if _v > 0 else "#B4553F"), family="monospace")
    ax.text(_bx + 30, _by, _layer_of(_w), fontsize=9.5, va="center",
            color=_LAYER_COL[_layer_of(_w)])
ax.add_patch(mpatches.FancyBboxPatch(
    (7, 5), 86, 12, boxstyle="round,pad=0,rounding_size=2",
    facecolor="white", edgecolor=GREEN, lw=1.8))
ax.text(11, 11, "combined, negation- and intensity-aware  →",
        fontsize=11.5, color=NAVY, va="center")
ax.text(64, 11, f"{_score:+.2f}", fontsize=17, fontweight="bold",
        color=GREEN, va="center")
ax.text(76, 11, f"bullish, well clear\nof the {BULL_CUTOFF:+.2f} cutoff",
        fontsize=9.8, color=GREY, va="center")

fig.suptitle("From one post to four numbers — what the machine reads, "
             "and what it keeps", fontsize=19, fontweight="bold", y=0.965)
fig.text(0.5, 0.925, "a real gold post, run through the shipped code: "
                     "every ticker, tag, lexicon count and score below is "
                     "computed at render time, not typed in",
         ha="center", fontsize=12.5, color=GREY)
foot(fig, "The desk machine never stores what anyone wrote. Text enters "
          "at step 1, is reduced to a date, a name, a tag and a number at "
          "step 2, and is never written to a committed file — which is "
          "why the whole\nhistory can be shared while the crowd stays "
          f"anonymous. Sentiment reads the first {TRUNCATE_CHARS} "
          "characters only: mood saturates early, and the tail costs "
          "time without adding signal.", y=0.012)
save(fig, "P13_one_post_end_to_end.png")

# =====================================================================
# 14 — the Louvain community map: who talks to whom, and how it goes
# =====================================================================
# NEW 2026-08-12 (desk: "how about the louvain communities map? like
# recreating that and make it look nice and interpretable").
#
# WHY THE OLD ONE WAS NOT INTERPRETABLE. Notebook 05 draws the k-core
# backbone with one dot per author, coloured by Louvain community. It is
# a hairball: the colours prove communities EXIST but say nothing about
# what any of them IS, so the only readable claim is "not one blob".
#
# WHAT THIS DOES INSTEAD. Two moves, both aggregations:
#   (1) one bubble per community rather than one dot per person, laid out
#       by who replies to whom, so the picture has seven readable objects
#       instead of a hundred thousand;
#   (2) every community is NAMED by what its members actually call - and
#       named from OVER-REPRESENTATION (a ticker's share inside the
#       community against its share across all of them), not raw volume.
#       Raw volume names five of the seven "SPY, MSFT, NVDA", because the
#       mega-caps dominate everywhere; lift separates them cleanly.
# The names themselves are desk-written English for what the signature
# tickers plainly are. The tickers are printed next to every name so the
# label can be checked rather than trusted.
print("14   louvain community map (~30s) ...", flush=True)
from analytics import influence_graph as ig14                     # noqa: E402

_iedges = pd.read_parquet(os.path.join(INF_DIR, "reply_edges.parquet"))
_g14 = ig14.build_graph(_iedges)
_comm = ig14.louvain(_g14)
_crep = ig14.community_report(_g14, _comm)
_csz = _comm.value_counts()
N_COMM = 7
_top = list(_csz.head(N_COMM).index)

_cc = icalls.assign(comm=icalls["author"].map(_comm)).dropna(subset=["comm"])
_cc["date"] = pd.to_datetime(_cc["date"])
_base = _cc["ticker"].value_counts(normalize=True)
_cj = _cc[_cc["fwd_ret"].notna()].copy()
_cj["mkt"] = _cj["date"].map(mkt_med)
_cj = _cj[_cj["mkt"].notna()]
_cj["ex"] = (_cj["fwd_ret"] - _cj["mkt"]) * _cj["direction"]

# Desk-written names, keyed by the signature tickers printed beside them.
# Keyed on the SIGNATURE rather than the community id because Louvain ids
# are not stable across runs - a new pull renumbers them, and a figure
# whose labels are pinned to an id would silently mislabel itself.
NAMEBOOK = [
    (("HYSA", "VTI", "SGOV", "ITOT", "SCHB", "BNDW"),
     "Index funds & savings"),
    (("MSTR", "IBIT", "FBTC", "STRC", "DYOR"), "Bitcoin proxies"),
    (("CLOV", "WKHS", "CLNE", "SPCE", "BNGO", "WISH"),
     "2021 meme stocks & SPACs"),
    (("IBKR", "TQQQ"), "Leveraged index & brokers"),
    (("VEEV", "MELI", "SPGI", "ADBE", "INTU", "ASML"),
     "Quality large-cap pickers"),
    (("IREN", "RKLB", "SPCX", "AMZN"), "Mega-cap & speculative mix"),
    (("SNXX", "SMCI", "ORCL", "INTC", "KORU"), "High-volume active traders"),
]
_used = set()
_rows = []
for _k in _top:
    _d = _cc[_cc["comm"] == _k]
    _n = _d["ticker"].value_counts()
    _sh = _d["ticker"].value_counts(normalize=True)
    _cand = _sh[_n >= 25]
    _lift = (_cand / _base.reindex(_cand.index)).sort_values(ascending=False)
    _sig = list(_lift.head(8).index)
    _name, _best = f"community {int(_k)}", 0
    for _keys, _label in NAMEBOOK:
        _hit = len(set(_keys) & set(_sig))
        if _hit > _best and _label not in _used:
            _name, _best = _label, _hit
    _used.add(_name)
    _ex = _cj[_cj["comm"] == _k]["ex"]
    _rows.append({
        "comm": int(_k), "name": _name, "members": int(_csz[_k]),
        "sig": ", ".join(_sig[:3]),
        "calls": len(_d), "judged": len(_ex),
        "ex": 100 * _ex.mean() if len(_ex) else np.nan,
        "yr": int(_d["date"].dt.year.median()),
    })
CM = pd.DataFrame(_rows).sort_values("ex")

# inter-community reply volume, for the edges on the map
_e = _iedges.assign(cu=_iedges["replier"].map(_comm),
                    cv=_iedges["author"].map(_comm)).dropna(
                        subset=["cu", "cv"])
_e = _e[_e["cu"].isin(_top) & _e["cv"].isin(_top)]
_pair = (_e.assign(a=np.minimum(_e["cu"], _e["cv"]),
                   b=np.maximum(_e["cu"], _e["cv"]))
         .groupby(["a", "b"]).size().reset_index(name="w"))
_intra = _pair[_pair["a"] == _pair["b"]]["w"].sum()
_inter = _pair[_pair["a"] != _pair["b"]]["w"].sum()
_links = _pair[_pair["a"] != _pair["b"]].sort_values("w", ascending=False)

# SEEDED SPRING LAYOUT on the seven-node community graph, then a HARD
# NON-OVERLAP PASS. Deterministic on purpose: a layout that moves between
# runs cannot be cited, the same reason notebook 05 seeds its own.
# The second pass is not cosmetic - a force layout treats nodes as points,
# but these nodes are bubbles up to 0.33 wide, so a "converged" layout
# still buried the 26k bubble under two of its neighbours. Radius-aware
# separation is what makes the picture readable.
_idx = {c: i for i, c in enumerate(CM["comm"])}
_W = np.zeros((len(CM), len(CM)))
for _r in _links.itertuples():
    _i, _j = _idx[int(_r.a)], _idx[int(_r.b)]
    _W[_i, _j] = _W[_j, _i] = _r.w
_W = _W / max(_W.max(), 1)
_smax = CM["members"].max()
_RAD = np.array([0.14 + 0.19 * np.sqrt(m / _smax) for m in CM["members"]])
_rng = np.random.default_rng(7)
_P = _rng.normal(0, 1, (len(CM), 2))
for _it in range(500):                       # attraction / repulsion
    _delta = _P[:, None, :] - _P[None, :, :]
    _dist = np.sqrt((_delta ** 2).sum(-1)) + 1e-9
    _rep = (_delta / _dist[..., None] ** 3) * 0.40
    _att = -(_delta * _W[..., None]) * 0.030
    _P += (_rep + _att).sum(1) * (1 - _it / 500) * 0.5
    _P -= _P.mean(0)
_P /= max(np.abs(_P).max(), 1e-9)
_P *= 0.92
PAD = 0.055
for _ in range(400):                         # separation, to convergence
    _moved = False
    for _i in range(len(_P)):
        for _j in range(_i + 1, len(_P)):
            _d = _P[_j] - _P[_i]
            _dl = float(np.hypot(*_d)) or 1e-9
            _need = _RAD[_i] + _RAD[_j] + PAD
            if _dl < _need:
                _push = (_need - _dl) / 2 * (_d / _dl)
                _P[_i] -= _push
                _P[_j] += _push
                _moved = True
    if not _moved:
...
gs = fig.add_gridspec(1, 2, width_ratios=[1.18, 1], wspace=0.06,
                      left=0.03, right=0.985, top=0.855, bottom=0.10)
PALETTE = ["#1F6F5C", "#2E6FD9", "#0A2540", "#5B8DEF", "#7A8CA3",
           "#B4553F", "#9DB9DC"]
COLS = {r.comm: PALETTE[i % len(PALETTE)]
        for i, r in enumerate(CM.itertuples())}

# ---- LEFT: the map ---------------------------------------------------
ax = fig.add_subplot(gs[0, 0])
_lim = float(np.abs(_P).max() + _RAD.max() + 0.16)
ax.set_xlim(-_lim, _lim)
ax.set_ylim(-_lim, _lim)
ax.set_aspect("equal")        # or every bubble renders as an ellipse
ax.axis("off")
for _r in _links.itertuples():
    _i, _j = _idx[int(_r.a)], _idx[int(_r.b)]
    ax.plot([_P[_i, 0], _P[_j, 0]], [_P[_i, 1], _P[_j, 1]],
            color=GREY, lw=0.5 + 6.0 * _r.w / _links["w"].max(),
            alpha=0.28, zorder=1, solid_capstyle="round")
# NO TEXT LABELS ON THE MAP AT ALL - the key IS the right-hand panel.
#
# Three labelling schemes were tried on this figure. Radiating each label
# from its own bubble collides for the ones near the middle, whose
# outward direction is short and ambiguous. Moving them to a ring with
# leader lines fixed that, but the right-hand labels then ran out of the
# left axes and printed on top of the right panel's tick labels, because
# matplotlib does not clip text to its axes and nothing in the layout
# knows how wide a string will be.
#
# The fix is to stop labelling twice. Every name is ALREADY written down
# the right panel, so repeating it on the bubbles was redundant even when
# it fitted. The two panels are joined by COLOUR, and - so the link needs
# no guessing - by the member count, printed inside each bubble and again
# beside its bar. Nothing can overlap because nothing is placed by hand.
for _i, _r in enumerate(CM.itertuples()):
    _k = _idx[_r.comm]
    _x, _y = _P[_k]
    ax.add_patch(mpatches.Circle((_x, _y), _RAD[_k], color=COLS[_r.comm],
                                 alpha=0.93, zorder=3, ec="white", lw=2.6))
    ax.text(_x, _y, f"{_r.members / 1000:.0f}k", ha="center", va="center",
            color="white", fontsize=15, fontweight="bold", zorder=4)
ax.set_title("Seven groups, and who replies to whom", fontsize=15,
             loc="left", pad=10)
ax.text(-_lim + 0.02, -_lim + 0.04,
        f"bubble = one community (area ∝ members) · line = replies "
        f"between two communities\n"
        f"{100 * _intra / max(_intra + _inter, 1):.0f}% of all replies "
        f"stay INSIDE a community — the groups are real, not a drawing "
        f"choice",
        fontsize=10.5, color=GREY, va="bottom")

# ---- RIGHT: what it costs to belong ---------------------------------
ax = fig.add_subplot(gs[0, 1])
_y = np.arange(len(CM))
ax.barh(_y, CM["ex"], color=[COLS[c] for c in CM["comm"]], height=0.62)
ax.axvline(0, color=NAVY, lw=2.0, zorder=4)
ax.set_yticks(_y, [r.name for r in CM.itertuples()], fontsize=12.5)
# tick labels carry the bubble colour - that is the whole key to the map
for _t, _r in zip(ax.get_yticklabels(), CM.itertuples()):
    _t.set_color(COLS[_r.comm])
    _t.set_fontweight("bold")
_span = max(CM["ex"].max(), 0) - min(CM["ex"].min(), 0)
for _i, _r in enumerate(CM.itertuples()):
    _off = -_span * 0.02 if _r.ex < 0 else _span * 0.02
    ax.text(_r.ex + _off, _i - 0.10, f"{_r.ex:+.1f}%", va="center",
            ha="right" if _r.ex < 0 else "left", fontsize=12.5,
            fontweight="bold", color=NAVY)
    ax.text(_span * 0.03, _i + 0.29,
            f"{_r.members / 1000:.0f}k members · {_r.sig} · "
            f"{_r.judged:,} judged calls",
            fontsize=9.5, color=GREY, va="center")
ax.set_xlim(CM["ex"].min() - _span * 0.30, max(CM["ex"].max(), 0)
            + _span * 0.60)
ax.set_xlabel("what their calls did over the next 20 days, "
              "relative to the market that day (%)", fontsize=11.5)
ax.set_title("The group you are in is worth more than the "
             "person you are", fontsize=15, loc="left", pad=10)
despine(ax)
ax.invert_yaxis()

fig.suptitle("The crowd is not one crowd — it is seven, and they do "
             "measurably different things", fontsize=19,
             fontweight="bold", y=0.965)
fig.text(0.5, 0.905,
         f"Louvain communities over the {_g14.n:,}-author reply graph "
         f"(modularity Q = {_crep['modularity']:.2f}); the seven largest "
         f"hold {100 * _csz.head(N_COMM).sum() / _g14.n:.0f}% of everyone",
         ha="center", fontsize=12.5, color=GREY)
foot(fig, "Communities are found from REPLY STRUCTURE ALONE — who "
          "answers whom — with no reference to tickers, themes or "
          "returns. The names are then read off what each group actually "
          "talks about, using each\nticker's share INSIDE the community "
          "against its share across all of them, so the mega-caps that "
          "dominate everywhere do not name five groups the same thing. "
          "That the groups then differ this much on the\nmeasured record "
          "is a finding, not a construction. TWO CAVEATS. The meme/SPAC "
          "group is a 2021 cohort where the others are 2026, so its "
          "figure carries that market as well as its behaviour — but the "
          "ordering\nsurvives inside 2026 alone (worst −1.8%, best "
          "+0.4%). And the names are a desk reading of the signature "
