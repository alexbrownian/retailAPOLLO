...
| `etf_constituents.csv` | the researched holdings behind `theme_tickers.csv`, and Each row carries its rank and a `verified` flag | when a fund rebalances — MTUM is semiannual, and its holdings ARE the momentum theme, so a stale list quietly makes the theme wrong | FULL rebuild |
...
| Test suite | `python -m pytest tests/ -q` — expect **155 passed, 2 skipped** | before any commit |
