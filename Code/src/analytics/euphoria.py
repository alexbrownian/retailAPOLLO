...
def _mention_share(counts_long, entity_col, name, all_days,
    ...
    The share is the robust construction in src/analytics/robust_share.py:
...
def build_all_series(prices: pd.DataFrame) -> list:
    ...
    from src.analytics.loaders import TICKER_COUNTS_BY_SOURCE
        ...
        if research:
            ...
            if _lag is not None:
                print(f"  NOTE: data now reaches {data_max_year}; the "
                      ...
                      "deliberately: src.analytics.run_analytics --what "
                      "euphoria --research")
