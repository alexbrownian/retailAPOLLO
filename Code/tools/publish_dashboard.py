...
published to `Data/dashboard/`, which is committed to the repository the
same way `Data/abstracted/` is.

Run this after a pipeline run, in either mode, then commit and push:

    python Code/update_data.py                 # the ordinary refresh
    python Code/tools/publish_dashboard.py     # stage the display bundle
...
WHY A BUNDLE AND NOT A CLOUD RECOMPUTE
    ...
    `src.analytics.run_analytics` on the host, which (a) needs
...
TEXT-FREE GUARD
    ...
    it is written, the same rule that protects `Data/abstracted/`. A
...
USAGE
    python Code/tools/publish_dashboard.py                # stage the bundle
    python Code/tools/publish_dashboard.py --dry-run      # report, write nothing
    python Code/tools/publish_dashboard.py --with-influence
"""
...
#
# FORBIDDEN_COLS protects Data/abstracted, whose files are aggregations of
...
# Data/abstracted rule in src/config.py is unchanged.
...
def publish(with_influence: bool = False, dry_run: bool = False,
    ...
    for name in DOCS_RESEARCH_FILES:
        stage(os.path.join(DATA_DIR, "research_record", name), name)
    ...
    if not dry_run and per_file:
        log("\nnext:")
        log("  git add Data/abstracted Data/dashboard")
