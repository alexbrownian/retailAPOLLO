...
| Switch signal strictness | NOTHING TO RUN — sidebar "signal setting" on the dashboard: **Standard** (F1 cut) or **Strict** (F0.5 cut, ~half the alerts). Both settings fire SHAPED calls (2026-08-09): GET IN only before a name's boom completes, GET OUT only after (the ground truth's own 120d bar), deep re-arm, one call per name per quarter per side, never a GET IN within 21d of a GET OUT. Evidence: `docs/research/alert_shape_sweep.json` |
