...
class TestResolve:
    def test_configured_theme_matches_before_yahoo(self):
        out = L.search("gold", remote=lambda q, l: [])
        assert out and out[0].symbol == "GLD" and out[0].origin == "approved"

    def test_yahoo_results_are_appended_and_deduplicated(self):
        out = L.search("brazil", remote=_yahoo)
...
class TestHoldingsAndWords:
    ...
    def test_yahoo_dash_class_becomes_dot(self):
    ...
    def test_price_is_cached_and_prefers_the_store(self, tmp_path, monkeypatch):
        ...
        def fetch(sym, a, b):
            ...
            return pd.DataFrame({"date": pd.to_datetime(["2026-01-02"]),
                                 ...
                                 "source": ["yfinance"]})
        a = L.price("ZZZZ", "20260101", "20260110", fetch=fetch)
        b = L.price("ZZZZ", "20260101", "20260110", fetch=fetch)
...
def _tiny_train(n=400, seed=0):
    import numpy as np
    from analytics.ml_detector import DESK_ML_BANK
...
class TestModelBundle:
    ...
    def test_the_pipeline_saves_the_bundle_and_publishes_it(self):
        src = (ROOT / "analytics" / "euphoria_phases.py").read_text(encoding="utf-8")
...
class TestBloombergFirst:
    ...
    def test_yahoo_is_used_when_no_terminal(self, monkeypatch):
        monkeypatch.setattr(L, "bloomberg_available", lambda refresh=False: False)
        called = []
        monkeypatch.setattr(L, "_yahoo_search", lambda q, l: called.append("y") or [])
        monkeypatch.setattr(L, "_bloomberg_search",
                            lambda q, l: (_ for _ in ()).throw(AssertionError("no")))
        L._remote_search("india", 5)
        assert called == ["y"]

    def test_bloomberg_failure_falls_through_to_yahoo(self, monkeypatch):
        ...
        monkeypatch.setattr(L, "_yahoo_search", lambda q, l: order.append("y") or [])
        L._remote_search("india", 5)
        assert order == ["b", "y"]
    ...
    def test_bloomberg_answer_wins_when_it_has_rows(self, monkeypatch):
        ...
        monkeypatch.setattr(L, "_yahoo_holdings",
                            lambda s: (_ for _ in ()).throw(AssertionError("no")))
