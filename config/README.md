...
| `app_credit` | text | Optional credit line. Empty shows nothing. |
...
### `approved_instruments.csv` — `symbol,bloomberg,name,note`
...
`pull_prices.py` (`--provider bloomberg`). The `yfinance` column is the
Yahoo Finance symbol used by `--provider yfinance` for non-US lines
(`1622.T`, `159915.SZ`); US tickers need no entry. Any other price provider must supply a
`prices.parquet` keyed on `symbol` (see `docs/DATA.md`).
