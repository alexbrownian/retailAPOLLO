...
def fig_theme_pulse(share, sent, px, symbol, title,
                    ...
                    zero_line=False):
    """THE PM CHART (desk 2026-08-07): price, attention and sentiment on
    ONE graph, none of them noisy - so "price up + attention up +
    sentiment up" is a single glance, not three tabs.

    Three axes, three registers:
      left   attention share of chatter (%, coverage-robust estimator -
             see analytics/robust_share.py; the denoising is in the
             estimator, not a cosmetic smooth)
      right  the anchor ETF's price (grey, context)
      far right  net-bullish sentiment (-1..+1, 28d post-weighted; the
             faint band marks +/-0 so 'the mood flipped' is visible)
    """
    fig = _base_fig(title)
    has_gaps = share.isna().any() and share.notna().any()
    if has_gaps:
        interp = share.interpolate(limit_area="inside")
        gap = share.isna()
        ...
        if bridge.notna().any():
            fig.add_trace(go.Scatter(
                x=bridge.index, y=bridge.values,
                name="not enough posts that week (dotted)",
                line=dict(color=BLUE, width=1.2, dash="dot"),
                opacity=0.5, hoverinfo="skip"), secondary_y=False)
    fig.add_trace(go.Scatter(x=share.index, y=share.values,
                             name=att_label,
    ...
    if sent is not None and sent.notna().any():
        fig.add_trace(go.Scatter(
            x=sent.index, y=sent.values, yaxis="y3",
            name="sentiment (net-bullish, 28d)",
            line=dict(color=BULL, width=1.6)))
        smin = float(sent.min(skipna=True))
        fig.update_layout(yaxis3=dict(
            overlaying="y", side="right", anchor="free", autoshift=True,
            range=[min(-1.0, smin), 1.0], showgrid=False,
            zeroline=True, zerolinewidth=1, zerolinecolor=INK_LABEL,
            title=dict(text="net-bullish", font=dict(size=10)),
            tickfont=dict(size=9)))
    fig.update_yaxes(title_text=att_axis, secondary_y=False)
    fig.update_yaxes(title_text="price (USD)", secondary_y=True)
    return fig
...
# SIGNAL SETTING (desk request 2026-08-09: "a stricter one - it fires a
# bunch"). Standard = the F1 cut; Strict = the F0.5 cut (precision
# weighted twice - the textbook strict operating point), both frozen
# walk-forward by the pipeline and both fired on UPWARD CROSSINGS only.
# Walk-forward record: Strict roughly halves alerts and false alarms
# (GET OUT 169 alerts / 0.18 FA-iy vs 305 / 0.35) at the cost of
# catching ~30% of episodes instead of ~42% - the evidence table is
...
_sig_mode = st.sidebar.radio(
    ...
    help="Standard balances catching episodes against false alarms "
         ...
         "as heavily (the F0.5 cut): about half the calls, half the "
         "false alarms, and fewer caught episodes. Both cuts are "
         "chosen from past years by the pipeline - never by hand.")
...
def render_euphoria_tab(kind, kind_label, key_prefix):
    ...
    if use_desk and _mdl != "rules":
        ...
        st.caption(f"Signal engine: the **{_mdl_words}** — tournament-"
                   ...
                   "fired on upward crossings only (a hot score cannot "
                   "re-fire all the way up a rally). The comparison "
                   "table is under 'what is euphoria?' below; records: "
                   "notebook 03 §SS, docs/research/ml_tournament.md, "
                   "docs/research/conditioning_sweep.json.")
...
if active_tab == "[dev] Data Stats":
    ...
    st.caption(f"Desk model: **{_mdl_ds}** | Standard cuts (F1): GET IN "
               ...
               "trigger: upward crossings, 21d cooldown | records: "
