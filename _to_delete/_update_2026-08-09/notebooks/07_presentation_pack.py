...
# | 6.5 | The flip-flop fix: trigger, smoothing, and the Strict setting | S6c |
...
# | Is it better than the old rules? | Yes, on every axis that matters: ~3.5–6× the episodes caught at double the score quality, and price actually BEHAVES like a top/start after its calls — after the old rules' GET OUT calls price kept rising. | §8 |
# | Does it flip-flop? | Fixed (2026-08-09): alerts fire on UPWARD CROSSINGS of the cut only (a hot score can no longer re-fire every cooldown all the way up a rally — IN/OUT flips −24%), and a **Strict** setting (F0.5 cut) halves the alert volume again. Score smoothing was tested and REJECTED (cost 10pts of GET OUT capture). | §6.5 |
...
# %% [markdown]
# ---
# # Q6.5 — The flip-flop fix (desk request 2026-08-09)
#
# **The complaint:** the signals flipped between GET IN and GET OUT too
# quickly, and GET OUT calls appeared repeatedly all the way up a long
# rally. **The cause:** a *level* trigger — any day with score ≥ cut
# fires (subject only to the 21d cooldown), so a persistently-hot score
# re-fires every cooldown for as long as a rally lasts.
#
# **Three candidate fixes, all tested walk-forward** on the frozen
# winner's scores (the model is NOT retrained — only how its daily score
# becomes an alert): trailing 7-day score smoothing (the desk's own
# suggestion; reuses the existing ROLL constant), a stricter F0.5 cut,
# and a **crossing trigger** (fire only on an upward crossing; the score
# must first drop below the cut to re-arm).
#
# **Pre-stated adoption rule:** fewest IN/OUT flips (a GET IN and a GET
# OUT on the same name within 30 days) among configurations that cut
# false alarms while keeping episode capture within 5 points of the
# shipped config, per head; one conditioning family for both heads.

# %%
_ct_rows = []
for _head in ("GET OUT", "GET IN"):
    for _k, _r in COND["results"][_head].items():
        _ct_rows.append({
            "head": _head, "config": _k,
            "caught": f"{_r['captured']}/{_r['detectable']} "
                      f"({_r['capture_rate']:.0%})",
            "false alarms": _r["false_alarms"],
            "FA/instr-yr": _r["fa_per_iy"],
            "precision": _r["precision"],
            "episode F1": _r["episode_f1"],
            "alerts": _r["n_alerts"],
            "IN/OUT flips": _r["flips_within_30d"],
        })
display(pd.DataFrame(_ct_rows).set_index(["head", "config"]))

# the verdict, applied mechanically
_ship = {h: COND["results"][h]["raw|F1.0|level"]
         for h in ("GET OUT", "GET IN")}
_elig = []
for _k in COND["results"]["GET OUT"]:
    ok = all(
        COND["results"][h][_k]["false_alarms"]
        < _ship[h]["false_alarms"]
        and COND["results"][h][_k]["capture_rate"]
        >= _ship[h]["capture_rate"] - 0.05
        for h in ("GET OUT", "GET IN")) and _k != "raw|F1.0|level"
    if ok:
        _elig.append(_k)
_verdict = (min(_elig, key=lambda k: COND["results"]["GET OUT"][k]
            ["flips_within_30d"]) if _elig else "raw|F1.0|level")
print(f"eligible under the pre-stated rule: {_elig}")
print(f"ADOPTED for the Standard setting: {_verdict}")
print("REJECTED: score smoothing - smooth|F1.0|crossing loses "
      f"{100*(_ship['GET OUT']['capture_rate'] - COND['results']['GET OUT']['smooth|F1.0|crossing']['capture_rate']):.0f} "
      "points of GET OUT capture (10 episodes) for a smaller flip "
      "reduction than the crossing trigger alone.")
print("The STRICT dashboard setting is the same conditioning at the "
      "F0.5 cut: "
      f"{COND['results']['GET OUT']['raw|F0.5|crossing']['n_alerts']} + "
      f"{COND['results']['GET IN']['raw|F0.5|crossing']['n_alerts']} "
      "alerts all-time vs "
      f"{COND['results']['GET OUT']['raw|F1.0|crossing']['n_alerts']} + "
      f"{COND['results']['GET IN']['raw|F1.0|crossing']['n_alerts']} "
      "standard.")

# %%
# S6c — before / after, on the name with the worst repeated-fire problem
from src.config import EUPHORIA_COOLDOWN_DAYS as _CD


def _alerts_variant(head, name, beta=1.0, trigger="crossing"):
    """Walk-forward alerts for one name under a conditioning variant.
    Cuts: the frozen per-year F1 cuts for beta=1; per-year F0.5 cuts
    recomputed under the same convention for beta=0.5."""
    d = WF[(WF["head"] == head) & (WF["name"] == name)]
    out = []
    for y, g in d.groupby("test_year"):
        thr = (THR[head].get(int(y)) if beta == 1.0
               else _strict_cuts.get((head, int(y))))
        if thr is None:
            continue
        ...
        if trigger == "level":
            al = _alerts_int(days, sc, thr)
        else:
            al, last, armed = [], None, True
            for dd, vv in zip(days, sc):
                if vv >= thr:
                    if armed and (last is None or dd - last >= _CD):
                        al.append(dd)
                        last = dd
                    armed = False
                else:
                    armed = True
            al = np.asarray(al, dtype=np.int64)
        out += list(np.asarray(al, dtype="datetime64[D]"))
    return pd.DatetimeIndex(out)


# per-year strict cuts (F0.5), same convention as production
_strict_cuts = {}
for _head, _label, _mode in (("GET OUT", "y_top", "top"),
                             ("GET IN", "y_onset", "onset")):
    for _y in sorted(WF[WF["head"] == _head]["test_year"].unique()):
        _tr = cand_px[cand_px["year"] < _y]
        if _tr.empty or _tr[_label].nunique() < 2:
            continue
        _fit = mld.make_ens_fit(_label)
        _trs = _tr.assign(score=_fit(_tr, _tr, mld.DESK_ML_BANK))
        _strict_cuts[(_head, int(_y))] = mld._choose_threshold_fbeta(
            _trs, episodes, _mode, beta=0.5)
print(f"strict cuts derived for {len(_strict_cuts)} (head, year) folds")

# the worst offender under the OLD level trigger, zoomed to its
# busiest 18 months (the repeated-fire problem is a within-rally
# phenomenon - a 9-year axis dilutes it)
_worst = max(WF["name"].unique(),
             key=lambda n: len(_alerts_variant("GET OUT", n,
                                               trigger="level")))
_sym_w = sym_by[_worst]
_pxw = pxmap[_sym_w].dropna()
_a_old = _alerts_variant("GET OUT", _worst, trigger="level")
_a_new = _alerts_variant("GET OUT", _worst, trigger="crossing")
_a_str = _alerts_variant("GET OUT", _worst, beta=0.5,
                         trigger="crossing")
...
def _inwin(al):
    return [a for a in al if _w0 <= a <= _w1]
...
for ax, al, ttl, c in (
        (axes[0], _a_old, f"BEFORE — level trigger: {len(_inwin(_a_old))}"
         ...
         "rally", GREY),
        (axes[1], _a_new, f"AFTER — crossing trigger (Standard): "
         f"{len(_inwin(_a_new))} calls", BLUE),
        (axes[2], _a_str, f"AFTER — crossing + F0.5 cut (Strict): "
    ...
    for r in episodes[(episodes.name == _worst)].itertuples():
        if r.peak >= _pxw.index.min() and r.trough <= _pxw.index.max():
            ax.axvspan(r.trough, r.peak, color=SKY, alpha=0.28, lw=0)
    _al_w = [a for a in al if _pxw.index.min() <= a <= _pxw.index.max()]
    ...
    despine(ax)
fig.suptitle(f"The flip-flop fix, on the worst offender — {_worst} "
             f"({_sym_w})", fontsize=16, fontweight="bold", y=1.0)
fig.text(0.01, -0.03,
         "A level trigger re-fires every cooldown for as long as the score stays hot; the crossing trigger requires the score to drop\n"
         "below the cut and cross UP again — one call per push, not one per 21 days. Walk-forward, across every name: IN/OUT flips "
         f"{COND['results']['GET OUT']['raw|F1.0|level']['flips_within_30d']} → "
         f"{COND['results']['GET OUT']['raw|F1.0|crossing']['flips_within_30d']} (Standard) → "
         f"{COND['results']['GET OUT']['raw|F0.5|crossing']['flips_within_30d']} (Strict), with capture within 2 points on both heads.",
         fontsize=11, color=GREY, va="top")
fig.tight_layout()
...
# (no tuned mixing weight); an upward crossing of the learned cut fires
# the call, one call per name per 21 days. Shown for the two cases the
...
def slide_case(disp, name, sym, w0, w1, fname):
    ...
    px = pxmap[sym].dropna().loc[w0:w1]
    a_out = [a for a in _alerts_variant("GET OUT", name)
             if w0 <= str(a.date()) <= w1]
    a_in = [a for a in _alerts_variant("GET IN", name)
            if w0 <= str(a.date()) <= w1]
    ...
    for ax, d, head, c, al in ((axes[2], d_out, "GET OUT", NAVY, a_out),
        ...
        ax.set_ylim(0, 1.05)
        ax.set_title(f"Step 2 — combine equally, fire on an UPWARD "
                     f"crossing of the trigger: {head}", fontsize=12.5,
                     loc="left")
...
# configuration — ensemble, crossing trigger, Standard cut.

# %%
# the shipped configuration's walk-forward record (crossing trigger)
SHIP = {h: COND["results"][h]["raw|F1.0|crossing"]
        for h in ("GET OUT", "GET IN")}
STRICT = {h: COND["results"][h]["raw|F0.5|crossing"]
          for h in ("GET OUT", "GET IN")}
...
# (the model's forward returns are recomputed on ITS shipped alerts -
# the crossing-trigger walk-forward set graded above)
_ship_alerts = {}
for _head in ("GET OUT", "GET IN"):
    _ship_alerts[_head] = {
        n: [pd.Timestamp(a) for a in
            _alerts_variant(_head, n, trigger="crossing")]
        for n in WF["name"].unique()}
FWD_SHIP = {h: mld.forward_returns(_ship_alerts[h], sym_by, pxmap)
            for h in ("GET OUT", "GET IN")}
...
# | 8 | Score smoothing vs stricter cut vs crossing trigger (§6.5, sweep of 8 configs) | **Crossing trigger** for Standard; **F0.5 cut** as the dashboard's Strict setting; smoothing REJECTED | Crossing cuts flips 24% at 2pts of capture; smoothing cost 10pts of GET OUT capture; the evidence is `conditioning_sweep.json` |
#
...
# - **Late GET INs exist** (alerts inside a real episode but past its
#   first 45 days). They are reported separately — inside a real rally,
#   but not "the start".
#
# **The PM line:** *the crowd, plus the tape, read by a two-model
# ensemble that only ever trains on the past, catches roughly two in
# five tops and one in two starts of every boom-bust episode our
# price-only judge defines — about four times what the hand rules
# caught — at about one false alarm per name every three to four years,
# fired once per push (not once per cooldown), with a Strict setting
# that halves the alert volume again, three numbers total to defend,
# and a comparison table for every alternative we declined.*
