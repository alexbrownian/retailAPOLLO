...
if active_tab == "AI Pulse":
    ...
    with _c1:
        if _pulse_real:
            st.caption(f"generated {_pulse.get('generated_at')} · model "
                       ...
                       f"{_pulse.get('as_of')} - written from the POSTS "
                       "alone: the model is shown no counts, shares or "
                       "model output, so everything here is a read of "
    ...
    if _pulse_real:
        ...
        if _cw or _dv:
            ...
            with _k2:
                if _dv:
                    st.markdown("### 5 - Where the crowd contradicts "
                                "itself")
                    for d in _dv:
                        st.markdown(f"- **{d.get('name', '?')}** - "
                                    f"{d.get('story', '')}")

        with st.expander("the measured numbers for the same week - the "
                         "model did NOT see these (the read is written "
                         "from posts alone; these are here so you can "
                         "check the story against them yourself)"):
