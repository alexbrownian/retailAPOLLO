...
Testing mode::
    ...
    python Code/ingestion/fetch_all.py --test              # Reddit: r/wallstreetbets newest 5
    python Code/ingestion/fetch_all.py --test --source x   # X: newest tweets for a few cashtags
...
3. Appends the new posts. The destination is picked automatically:
   ...
   * aggregates mode (no ``posts.parquet``, or ``--abstracted``):
     ``append_live_abstracted.py`` folds them into ``Data/abstracted`` as
...
``--abstracted``
    Force the ``Data/abstracted`` append (the aggregates-mode path).
``--serial``
...
def main():
    ...
    p.add_argument("--abstracted", action="store_true",
                   help="fold new posts into Data/abstracted (text-free "
    ...
    #   * Data/abstracted (aggregates mode - no raw store is kept;
    ...
    if args.no_merge:
        target = "Data/abstracted" if use_abstracted else "posts.parquet"
        ...
        print(f"\n--- APPEND: folding new posts into Data/abstracted ({why}) ---")
        rc = subprocess.run(
    ...
    print("\nsee what landed:  python Code/ingestion/check_live_ingestion.py")
