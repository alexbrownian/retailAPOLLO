...
from src.analytics.euphoria_phases import (episode_catalog, build_day_frame,
                                       ...
                                       boom_state_frame)
from src.analytics.loaders import (load, THEME_COUNTS, THEME_SENT,
...
def choose_threshold_strict(train_scored: pd.DataFrame,
                            ...
                            n_instruments: int) -> float:
    """The STRICT operating point - a stricter setting with fewer false
    alarms: identical machinery, F0.5 - the standard precision-weighted
    F-measure. Same walk-forward convention, same grid.

    beta is src/config.EUPHORIA_STRICT_BETA (0.5 by default): a NAMED,
    documented choice that a sweep can move, rather than a literal buried
    in a function body. See the config note for why, and for the rule
    that it must be chosen by a pre-stated criterion."""
    ...
    for head, label, mode in (("get_out", "y_top", "top"),
        ...
        for mname, maker in entries:
            ...
            for vname, vframe, vbank in variants:
                ...
                # The entry line prints immediately, the timing line
