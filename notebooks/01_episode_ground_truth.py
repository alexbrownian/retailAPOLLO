...
# |---|---|---|
# | How many euphoria episodes are there? | **494** boom→bust arcs across 52 priced instruments, 2017–2026 — under the ground truth LOWERED on 2026-08-07 (20/40 boom, 12/25 bust; the sweep that justified it is in §2's table source and notebook 03). | §3 |
# | How many can a crowd detector fairly be graded on? | **199** for the start, **219** for the top. The rest happened while the post archive was too thin to judge. | §3 |
# | Is the archive evenly spread? | **No, and this is the single biggest limitation in the project.** 2021 alone supplies 52 of the 199 gradeable starts; 2024 supplies one and 2025 **zero**. | §3, §7 |
# | How big is a typical episode? | Median boom **+49%**, median bust **−27%**, median run **93 days** low→top. | §4 |
...
#     * **G2** that peak sits ≥20% (theme) / ≥40% (single) above the lowest
#       close of the prior 120d;
#     * **G3** a fall of ≥12% / ≥25% follows within 90d.
...
#   ≥100 tagged posts in 28 days, the existing A0 rule, CONVENTION — held on at
...
#    that deflated slowly — no ≥12/25% fall inside 90 days — never enters the
...
# %% [markdown]
# ---
# # SS — How the bars were SET, not just stated: the 2026-08-07 sweep
#
# **WHY THIS**
#
...
#   judge on (a) which named episodes exist at each level, (b) whether the
#   model's threshold-free accuracy holds, (c) whether "episode" still
#   means anything (episodes per instrument-year).
#
# **HOW IT WORKS** — the sweep record is frozen in
# `docs/research/ml_tournament.json` (`ground_truth_sweep`), written by
# `python -m analytics.ml_detector --sweep`; this section renders it
# rather than re-running it (one fact, one home — re-derive it with that
# command, never by editing numbers here).

# %%
_sw = json.load(open(RESEARCH_DIR / "ml_tournament.json"))["ground_truth_sweep"]
_sw_rows = []
for _k, _v in _sw.items():
    _sw_rows.append({
        "definition": _k,
        "episodes": _v["episodes"],
        "episodes / instr-yr": round(_v["episodes"] / (52 * 9.5), 2),
        "GET OUT AP": _v["get_out"]["ap"],
        "GET OUT capture": f"{_v['get_out']['captured']}/{_v['get_out']['detectable']}",
        "GET OUT FA/iy": _v["get_out"]["fa_per_iy"],
        "GET IN AP": _v["get_in"]["ap"],
        "gold/meme/semis present": all(
            t in _v["themes_with_episode"]
            for t in ("gold_metals", "meme_stocks", "semiconductors")),
    })
...
# **SO WHAT**
#
# * **20/40, 12/25 adopted** (the middle row): episodes 292→494, every
#   named desk episode present, the model's AP *rises* (0.168→0.201 GET
#   OUT, 0.211→0.290 GET IN) — the extra episodes are learnable, not
#   noise.
# * **15/30 rejected, with the reason written down**: ~1.5 episodes per
#   instrument-year and 0.71 FA/iy means "euphoria episode" degrades into
...
# tuning?"** The sweep re-ran the ENTIRE walk-forward under each
# definition — models refit, thresholds re-chosen on train years only —
# so no test year ever leaked into a fit. What changed is the *question*
# ("catch mid-sized manias too"), and the record keeps all three levels
# so anyone can read what the other answers would have been.
